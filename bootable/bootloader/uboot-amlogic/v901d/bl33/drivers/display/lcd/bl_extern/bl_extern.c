...
static int bl_extern_set_level(unsigned int level)
{
	...
	level = dim_min - ((level - level_min) * (dim_min - dim_max)) /
			(level_max - level_min);
	//above formula is not right for local dimming disbled case
	//skip the calculation result and use the bl_extern_level
	if (bl_extern_driver.device_bri_update)
		bl_extern_driver.device_bri_update(bl_extern_level);
...
static int bl_extern_add_driver(void)
{
	...
	if (i2c_serdes_0_probe(&des_id) != 0)
		if (i2c_serdes_1_probe(&des_id) != 0)
			BLEXERR("Both i2c_serdes fail!!\n");

	BLEX("%s: des_id = %d\n", __func__, des_id);
	if (des_id == 948)
		ret = i2c_m0516ldn_probe();
	else if (des_id == 928)
		ret = i2c_a8552_probe();
