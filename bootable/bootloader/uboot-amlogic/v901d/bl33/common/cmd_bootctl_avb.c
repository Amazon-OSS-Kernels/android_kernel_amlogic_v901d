...
uint32_t ab_validate_crc32 (AvbABData* info) {
    return avb_htobe32(
...
{
    uint32_t computed_crc32 = ab_validate_crc32(info);

    if (info->crc32 != computed_crc32) {
        printf("Crc32: %d is incorrect. Expecting this instead: %d\n", info->crc32, computed_crc32);
        return false;
    }
...
// USB Update has a bug with marking the slot successful, so have this here to fix the bug on the
// first update after the USB, so car's don't need an OTA to properly USB update.
// The root cause is fixed below in do_MarkSlotSuccessful.
// Returns true if we had to normalize a slot, false if slot are okay.
bool boot_info_normalize(AvbABData *info, int slot)
{
    if (info->slots[slot].priority == AVB_AB_MAX_PRIORITY &&
        info->slots[slot].tries_remaining == AVB_AB_MAX_TRIES_REMAINING &&
        info->slots[slot].successful_boot == 1)
    {
        printf("Normalizing slot: %d\n", slot);
        info->slots[slot].tries_remaining = 0;
        dump_boot_info(info);
        return true;
    }
    return false;
    ...
    char * const argv[])
{
    ...
    printf("Resetting A/B boot-info\n");
...
static int do_DecrementRetryCount(
    cmd_tbl_t * cmdtp,
    int flag,
    int argc,
    char * const argv[])
{
    ...
    if (info.slots[active_slot].successful_boot != 1) {
        if (info.slots[active_slot].tries_remaining > 0) {
            info.slots[active_slot].tries_remaining--;
        } else {
            info.slots[active_slot].priority = 0;
            info.slots[active_slot].successful_boot = 0;
            other_slot = 1 - active_slot;
            info.slots[other_slot].priority = AVB_AB_MAX_PRIORITY;
        }
    }
    boot_info_save(&info, miscbuf);
...
int boot_info_mark_slot_successful(AvbABData* info, int slot)
{
    info->slots[slot].tries_remaining = 0;
    info->slots[slot].successful_boot = 1;
...
static int do_MarkSlotSuccessful(
    cmd_tbl_t * cmdtp,
    int flag,
    int argc,
    char * const argv[])
{
    ...
    if (strcmp(argv[1], "a") == 0) {
        printf("marking slot successful: a \n");
        boot_info_mark_slot_successful(&info, 0);
    } else if (strcmp(argv[1], "b") == 0) {
        printf("marking slot successful: b\n");
        boot_info_mark_slot_successful(&info, 1);
    } else {
    ...
    char * const argv[])
{
    ...
    if (boot_info_normalize(&info, 0) || boot_info_normalize(&info, 1)) {
        boot_info_save(&info, miscbuf);
    }
...
U_BOOT_CMD(
    decrement_retry_count, 2, 0, do_DecrementRetryCount,
    "decrement_retry_count",
    "\nThis command will decrement retry count\n"
    "So you can execute command: decrement_retry_count"
);

U_BOOT_CMD(
    reset_ab_metadata, 2, 0, do_ResetAbMetadata,
    "reset_ab_metadata",
    "\nThis command will reset_ab_metadata\n"
    "So you can execute command: reset_ab_metadata"
);

U_BOOT_CMD(
    mark_slot_successful, 2, 1, do_MarkSlotSuccessful,
    "mark_slot_successful",
    "\nThis command will mark the slot successful\n"
    "So you can execute command: mark_slot_successful a"
);
