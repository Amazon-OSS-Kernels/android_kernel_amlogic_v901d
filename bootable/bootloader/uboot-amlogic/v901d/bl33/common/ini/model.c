...
#define DEFAULT_MODEL_SUM_PATH_A "/tvconfig_a/model/model_sum.ini"
#define DEFAULT_MODEL_SUM_PATH_B "/tvconfig_b/model/model_sum.ini"

#define TVCONFIG_PATH "/tvconfig"
#define TVCONFIG_A_PATH "/tvconfig_a"
#define TVCONFIG_B_PATH "/tvconfig_b"
#define MODIFIED_PANEL_INI_STRING_LEN 100
...
static int handle_lcd_pwr(struct lcd_attr_s *p_attr)
{
	...
	for (i = 0; i < g_lcd_pwr_on_seq_cnt; i++) {
		...
		if ((tmp_base_ind + 3) >= LCD_PWR_BUF_SIZE_MAX) {
			ALOGE("%s: buf size is not enough\n", __func__);
			return -1;
		}
	...
	if (tmp_cnt >= LCD_PWR_BUF_SIZE_MAX) {
		ALOGE("%s: buf size is not enough\n", __func__);
		return -1;
	}
...
static int handle_panel_misc(struct panel_misc_s *p_misc)
{
	...
	if (strcmp(ini_value, "null") == 0) {
		strncpy(p_misc->outputmode, "1080p60hz",
...
const char *modify_ini_string(char* ini_string) {
	char *modified_ini_value = NULL;
	char *panel_ini_value;

	modified_ini_value = (char *)malloc(sizeof(char) * MODIFIED_PANEL_INI_STRING_LEN);
	if (modified_ini_value == NULL) {
		ALOGE("%s, malloc buffer memory error!!!\n", __func__);
		return ini_string;
	}

	panel_ini_value = strstr(ini_string, "/panel");

	memset(modified_ini_value, '\0', 100);
	if (active_slot == 'a') {
		strcpy(modified_ini_value, TVCONFIG_A_PATH);
	} else if (active_slot == 'b') {
		strcpy(modified_ini_value, TVCONFIG_B_PATH);
	} else {
		strcpy(modified_ini_value, TVCONFIG_PATH);
	}

	strcat(modified_ini_value, panel_ini_value);
	ALOGI("%s, modified PANELINI_PATH: %s\n", __func__, modified_ini_value);

	return modified_ini_value;
}

int parse_model_sum(const char *file_name, char *model_name)
{
	const char *ini_value = NULL;
	const char *modified_ini_value = NULL;
	...
	if (strcmp(ini_value, "null") != 0) {
		modified_ini_value = modify_ini_string(ini_value);
		setenv("model_panel", modified_ini_value);
	}
	else
...
const char *get_model_sum_path(void)
{
	...
	if (model_path == NULL) {
		if (active_slot == 'a')
			return DEFAULT_MODEL_SUM_PATH_A;
		else if (active_slot == 'b')
			return DEFAULT_MODEL_SUM_PATH_B;
		else
