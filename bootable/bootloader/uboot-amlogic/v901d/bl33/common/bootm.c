				   ...
				   ulong *os_data, ulong *os_len)
{
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_ANDROID:
		...
		/*Check the idme and decide USB mode*/
		char * cmd_line = (char *)((struct andr_img_hdr *)buf)->cmdline;
		if (*cmd_line) {
			android_image_update_usb_mode(cmd_line);
		} else {
			printf("Can't check the usb mode\n");
		}
