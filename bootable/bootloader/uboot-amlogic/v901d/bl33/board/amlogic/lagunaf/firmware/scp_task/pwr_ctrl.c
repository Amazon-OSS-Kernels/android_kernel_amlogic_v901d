...
static void power_off_at_24M(unsigned int suspend_from)
{
	...
	writel(readl(AO_RTI_PINMUX_REG0) & (~(0xf << 16)), AO_RTI_PINMUX_REG0);
...
static void power_on_at_24M(unsigned int suspend_from)
{
	...
	writel(readl(AO_RTI_PINMUX_REG0) & (~(0xf << 12)), AO_RTI_PINMUX_REG0);
#else
	/*set gpioAO_3 high to power on VDD5V and hdmi_3.3v*/
	//jetopto change to 10
	writel(readl(AO_GPIO_O) | (1 << 10), AO_GPIO_O);
	writel(readl(AO_GPIO_O_EN_N) & (~(1 << 10)), AO_GPIO_O_EN_N);
	writel(readl(AO_RTI_PINMUX_REG1) & (~(0xf << 12)), AO_RTI_PINMUX_REG1);
