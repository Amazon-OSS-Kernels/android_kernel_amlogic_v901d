...
extern void mute_audio(u8 value);
...
static u32 parse_reason(const char *cmd)
{
	...
	if (cmd) {
		if (strcmp(cmd, "recovery") == 0 ||
			...
			pr_info("soc_state_pwm [%s] nextboot is recovery, setting recovery lcr\n", __func__);
			...
			pr_info("soc_state_pwm [%s] nextboot is normal, mode index is %d\n", __func__, current_mode_index);
			if (current_mode_index == RECOVERY_MODE_INDEX) {
				pr_info("soc_state_pwm [%s] currentboot is recovery, setting postrecovery lcr\n", __func__);
				life_cycle_set_special_mode(LIFE_CYCLE_SMODE_POST_RECOVERY);
			} else { //MODE_POSTRECOVERY
				pr_info("soc_state_pwm [%s] currentboot is not recovery, setting warmboot lcr\n", __func__);
