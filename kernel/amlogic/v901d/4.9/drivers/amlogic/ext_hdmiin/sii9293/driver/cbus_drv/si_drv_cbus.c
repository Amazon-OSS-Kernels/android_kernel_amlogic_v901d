...
#define HDMI_SVD_STRUCTURE_LENGTH_EDID                                         \
	17 // number of VIC entries in EDID (should be updated per application)

ROM struct Mandatory3dFmt_t Mandatory_60[Mandatory3dFmt_60] = {
	{{VIC_1080P_24, 0}, {FRAME_SEQUENTIAL | TOP_BOTTOM} },
	{{VIC_720P_60, 0}, {FRAME_SEQUENTIAL | TOP_BOTTOM} },
	{{VIC_1080i_60, 0}, {LEFT_RIGHT} }
};

ROM struct Mandatory3dFmt_t Mandatory_50[Mandatory3dFmt_50] = {
	{{VIC_1080P_24, 0}, {FRAME_SEQUENTIAL | TOP_BOTTOM} },
	{{VIC_720P_50, 0}, {FRAME_SEQUENTIAL | TOP_BOTTOM} },
	{{VIC_1080i_50, 0}, {LEFT_RIGHT} }
};

union VIC3DFormat_t VIC3DFormats[HDMI_3D_SVD_STRUCTURE_LENGTH] = {
	{0},
};

struct CbusDrvInstanceData_t cbusDrvInstance;
struct CbusDrvInstanceData_t *pDrvCbus = &cbusDrvInstance;
...
{
	// Do not clear SiiCBUS_MHL_CONNECTION_CHG, since it will be processed
	// out of cbus handler.
...
/*****************************************************************************/
/**
 *  @brief        Returns if the peer is requesting for scratchpad write
 *permission
 *
 ...
 *  @retval    false        peer is not requesting for scratchpad write
 *permission
 *
 *****************************************************************************/
...
/*****************************************************************************/
/**
 *  @brief        Send out the 3D Write Burst information
 *
...
{
	struct Mhl2VideoFormatData_t *pvideoFormatData =
	    pDrvCbus->videoFormatData;
	// no need to check SpSupport since mhl source always support scatchpad
	// from spec.
	while (BURST_ID(pvideoFormatData->burstId) == burstId_3D_VIC ||
	       BURST_ID(pvideoFormatData->burstId) == burstId_3D_DTD) {
		if (!SiiCbusWritePeersScratchpad(0x00, 16,
						 (uint8_t *)pvideoFormatData)) {
			return ~SUCCESS;
		}
		pvideoFormatData++;
	}
...
/*****************************************************************************/
/**
 *  @brief        Returns if the peer requesting for 3D information
 *
 *  @return    peer has requested 3D information or not
 *  @retval    true        peer has requested 3D information
 *  @retval    false        peer has not requested 3D information
 *
 *****************************************************************************/
...
{
	// TODO: Please update data here per EDID in actual application.
	// Currently, default 3D formats are added

	// 02 / 03 / 04 / 05 / 06 here are indexes of corresponding VICs in
	// EDID's Video Data Block
...
void SiiDrvCbusBuild3DData(void)
{
	...
	memset(pDrvCbus->videoFormatData, 0x00,
	       sizeof(pDrvCbus->videoFormatData));
...
#ifdef HDMI_3D_SVD_SUPPORT
	{
		...
		j = 1;
		for (i = 0; i < length; i++) {
			pDrvCbus->videoFormatData[j].burstId.low =
			    burstId_3D_VIC;
			...
			pDrvCbus->videoFormatData[j]
			    .videoDescriptors[k]
			    .FrameSequential =
			    VIC3DFormats[i].Fields.FrameSequential;
			pDrvCbus->videoFormatData[j]
			    .videoDescriptors[k]
			    .TopBottom = VIC3DFormats[i].Fields.TopBottom;
			pDrvCbus->videoFormatData[j]
			    .videoDescriptors[k]
			    .LeftRight = VIC3DFormats[i].Fields.LeftRight;
			if (k == 4)
				j++;
		}
	}
	...
	for (i = 0; i < MaxVideoEntry; i++) {
		...
		checksum = 0;
		for (j = 0; j < sizeof(struct Mhl2VideoFormatData_t); j++)
			checksum += *(ptr + j);
...
/*****************************************************************************/
/**
 *  @brief        Returns the last VS cmd and data bytes retrieved by the CBUS
 *ISR.
 *
...
{
	SiiDrvSwitchDeviceRXTermControl(terminate ? SiiTERM_MHL
						  : SiiTERM_DISABLE);
}
 ...
 *command such as READ_DEVCAP, GET_VENDOR_ID, SET_HPD, CLR_HPD, etc.
 *
 *  @param[in]        pReq        Pointer to a cbus_req_t structure containing
...
bool SiiDrvInternalCBusWriteCommand(struct cbus_req_t *pReq)
{
	...
	DEBUG_PRINT(MSG_DBG, "CBUS:: Send MSC cmd %02X, %02X, %02X\n",
	...
	case MHL_GET_DDC_ERRORCODE: // 0x6A - Get DDC channel command error
				    // code.
	...
	case MHL_WRITE_BURST:
		...
		for (i = 0; i < pReq->length; i++) {
			SiiRegWrite(REG_CBUS_SCRATCHPAD_0 + i,
				    pReq->msgData[i]);
		}
...
bool SiiDrvCbusProcessInterrupts(void)
{
	...
	// Read CBUS interrupt status. Return if nothing happening on the
	// interrupt front
	...
	intStatus1 &= ~(BIT_HEARTBEAT_TIMEOUT | BIT_SET_CAP_ID_RSVD |
			BIT_CBUS_PKT_RCVD | BIT_CEC_ABORT);
	...
	if (intStatus0) {
		...
		if (intStatus0 & BIT_MSC_SET_INT_RCVD) {
			...
			if (intStatusTemp & BIT0) {
				pDrvCbus->statusFlags |=
				    SiiCBUS_DCAP_CHG_RECEIVED_FM_PEER;
			}
			...
			if (intStatusTemp & BIT2) {
				pDrvCbus->statusFlags |=
				    SiiCBUS_REQ_WRT_RECEIVED_FM_PEER;
			}
			if (intStatusTemp & BIT3) {
				pDrvCbus->statusFlags |=
				    SiiCBUS_GRT_WRT_RECEIVED_FM_PEER;
			}
			if (intStatusTemp & BIT4) {
				pDrvCbus->statusFlags |=
				    SiiCBUS_3D_REQ_RECEIVED_FM_PEER;
			}
			SiiRegWrite(REG_CBUS_SET_INT_0,
				    intStatusTemp); // Clear received interrupts
		}

		// This step is redundant as we do get DSCR_CHG interrupt up
		// there
		...
		if (intStatus0 & BIT_MSC_WRITE_STAT_RCVD) {
			...
			if (intStatusTemp & BIT0) {
				pDrvCbus->statusFlags |=
				    SiiCBUS_DCAP_RDY_RECEIVED_FM_PEER;
			}

			intStatusTemp = SiiRegRead(REG_CBUS_WRITE_STAT_1);
			if (intStatusTemp & BIT3) {
				pDrvCbus->statusFlags |=
				    SiiCBUS_PATH_EN_RECEIVED_FM_PEER;
			}
		}
		...
		if (intStatus0 & BIT_MSC_CMD_DONE) {
			pDrvCbus->statusFlags |= SiiCBUS_MSC_CMD_DONE;
			pDrvCbus->msgData0 =
			    SiiRegRead(REG_CBUS_PRI_RD_DATA_1ST);
			pDrvCbus->msgData1 =
			    SiiRegRead(REG_CBUS_PRI_RD_DATA_2ND);
		}
		...
		if (intStatus0 & BIT_CONNECT_CHG) {
			...
			pDrvCbus->busConnected =
			    SiiRegRead(REG_CBUS_BUS_STATUS) & BIT_BUS_CONNECTED;
		}
	}
	...
	if (intStatus1) {
		if (intStatus1 & BIT_DDC_ABORT) {
			pDrvCbus->ddcAbortReason =
			    SiiRegRead(REG_DDC_ABORT_REASON);
			pDrvCbus->statusFlags |= SiiCBUS_DDC_ABORT;
			SiiRegWrite(REG_DDC_ABORT_REASON,
				    pDrvCbus->ddcAbortReason);
		}

		// MSC_ABORT happened as Responder
		if (intStatus1 & BIT_MSC_ABORT_RES) {
			pDrvCbus->mscAbortResReason =
			    SiiRegRead(REG_MSC_ABORT_RES_REASON);
			pDrvCbus->statusFlags |= SiiCBUS_MSC_ABORT_RES;
			SiiRegWrite(REG_MSC_ABORT_RES_REASON,
				    pDrvCbus->mscAbortResReason);
		}

		// MSC_ABORT happened at this device itself
		if (intStatus1 & BIT_MSC_ABORT) {
			pDrvCbus->mscAbortReason =
			    SiiRegRead(REG_MSC_ABORT_REASON);
			pDrvCbus->statusFlags |= SiiCBUS_MSC_ABORT;
			SiiRegWrite(REG_MSC_ABORT_REASON,
				    pDrvCbus->mscAbortReason);
		}
		...
		if (intStatus1 & BIT_MHL_CONNECT_CHG) {
			pDrvCbus->statusFlags |= SiiCBUS_MHL_CONNECTION_CHG;
			pDrvCbus->mhlConnected =
			    SiiRegRead(REG_CBUS_BUS_STATUS) & BIT_MHL_CONNECTED;
		}
	}
...
bool SiiDrvCbusInitialize(void)
{
	...
	memset(pDrvCbus, 0, sizeof(struct CbusDrvInstanceData_t));

	// Setup local DEVCAP registers for read by the peer
	for (index = 0; index < (sizeof(cbusInitCbusRegsList) / 2);
	     index += 2) {
		SiiRegWrite(cbusInitCbusRegsList[index],
			    cbusInitCbusRegsList[index + 1]);
	}
	...
	SiiRegWrite(REG_CBUS_INTR_0_MASK, 0x00); // Enable desired interrupts
	SiiRegWrite(REG_CBUS_INTR_1_MASK, 0x00); // Enable desired interrupts
