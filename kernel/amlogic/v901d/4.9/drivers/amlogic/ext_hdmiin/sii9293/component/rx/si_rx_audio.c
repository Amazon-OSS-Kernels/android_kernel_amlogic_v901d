...
struct rx_audio_vars_type {
	...
	// uint8_t	status_received : 1; // true if channel status data is
	// valid
	uint8_t audio_info_frame_received : 1; // true if audio info frame has
						 // been received
	uint8_t encoded : 1;	 // false for PCM, true for encoded streams
	uint8_t protected : 1; // true for audio with protected context and
				 // with ACP packet
	// uint8_t	dsd_mode : 1; // true for DSD mode
	// uint8_t	start_request : 1; // a flag to start audio processing
	// uint8_t	exceptions_enabled : 1; // true if audio exceptions are
	// on, false otherwise (shadow bit of 0x60.0xB5.0) uint8_t
	// hbr_mode : 1; // true for HBR Audio mode uint8_t	muted : 1; //
	// set if Audio Mute pin is asserted by FW, cleared otherwise
	uint8_t new_countdown : 1; // a request for a new count down in
				     // RxAudioTimerHandler()
	uint8_t audio_is_on : 1;   // static variable used in switch_audio()
	uint8_t layout1 : 1;	     // true for layout 1; false for layout 0
	uint16_t count_down;	     // a countdown timer
	uint8_t measured_fs_code;    // calculated sample frequency in Channel
				     // Status's format
	uint8_t channel_status[5];   // first 5 bytes of audio status channel
};

static struct rx_audio_vars_type audio_vars = {0};

struct audio_fs_search_t {
...
static ROM const struct audio_fs_search_t audio_fs_list
[AUDIO_FS_LIST_LENGTH + 1] = {
...
static ROM const uint8_t audio_channel_mask[AUDIO_CHANNEL_MASK_TABLE_LENGTH] = {
	0x10, 0x30, 0x30, 0x30, 0x70, 0x70, 0x70, 0x70, 0x70, 0x70, 0x70,
	0x70, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xB0, 0xB0,
	0xB0, 0xB0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0,
};
...
static bool is_spdif_out_prohibited(void)
{
	return 0 != (audio_vars.protected || // protected audio
		     ((!audio_vars.encoded) &&
		      audio_vars.ca) // PCM with more than 2 channels
		     || !SiiSpdifEnableGet());
}
...
// Limitation: this function may return zero if it is called before video is
// detected.
static uint32_t get_tclk_10kHz(void)
{
	...
	case SII_RX_VBUS_WIDTH_36:// DC 36 bit
		...
		break;
	case SII_RX_VBUS_WIDTH_48:// DC 48 bit
		tmds_clk_10kHz *= 2; // *2
		break;
	}
#endif
...
{
	uint32_t mclk =
	    RX_CFG_256FS; // the minimum MCLK for TDM is 256*fs, set MCLK as
			  // 256*fs always
...

	{
		...
		if (tmds_clk_10kHz && cts) {
			...
			uint16_t fs_calculated_100Hz =
			    (tmds_clk_10kHz * n / cts) * 100 / 128;
			// Note: the order of operations is optimized for the
			// maximum precision. Overflow should not occur during
			// the operations. tmds_clk_10kHz - assume maximum value
			// 30,000 (15bit) for 300 MHz n - assume maximum value
			// 192kHz*128/300=81000 (17bit) tmds_clk_10kHz*n should
			// fit into 32 bits Maximum fs_calculated_100Hz is
			// 192000/100=1920 fits into 16bit

			// Find closest standard audio Fs.
			for (i = 0; i < AUDIO_FS_LIST_LENGTH; i++) {
				if ((fs_calculated_100Hz <=
				     (audio_fs_list[i].max_Fs)) &&
				    (fs_calculated_100Hz >
				     (audio_fs_list[i].min_Fs))) {
					// search if calculated Fs close to the
					// Fs in the table
					break;
				}
			}
		...
		if (fs_code_per_channel == AUDIO_CHST4__FS_UNKNOWN)
			DEBUG_PRINT(MSG_STAT, "RX Audio: Fs code = %02X\n",
				    (int)(audio_vars.fs));
		//else {
			//			DEBUG_PRINT(MSG_STAT, "RX Audio:
			//Calculated Fs = %d kHz\n", (int)
			//fs_frequency_per_channel/10);
		//}

		audio_vars.measured_fs_code = fs_code_per_channel;
	}

	SiiRegWrite(RX_A__FREQ_SVAL,
		    (mclk << 6) | (mclk << 4) | fs_code_per_channel);
...
static void set_output(void)
{
	...
	if (audio_vars.audio_info_frame_received)
		...
		// Encoded data can be whether through 1 I2S channel or through
		// HBRA; it cannot be through multiple I2S channels in non-HBRA
		// mode.

	SiiRegWrite(RX_A__I2S_CTRL2, out_mask);

	// Enable TDM as configured
	SiiRegBitsSet(RX_A__TDM_CTRL1, RX_M__TDM_CTRL1__TDM_EN,
		      SiiTdmEnableGet());

	// enable SPDIF if allowed
	SiiRegWrite(
	    RX_A__AUDRX_CTRL,
	    RX_M__AUDRX_CTRL__PASS_SPDIF_ERR | RX_M__AUDRX_CTRL__I2S_MODE |
...
{
	struct SiiRxAudioFormat_t rxAudioFormat;
	...
	memcpy(rxAudioFormat.audioStatusChannel, audio_vars.channel_status,
	       SI_AUDIO_ST_CH_LEN);
}
...
void RxAudio_OnChannelStatusChange(void)
{
	...
	audio_vars.fs = SiiRegRead(RX_A__PCLK_FS) &
			RX_M__PCLK_FS__SPDIF_EXTRACRTED_FS; // HW measured Fs

	if (audio_vars.measured_fs_code != AUDIO_CHST4__FS_UNKNOWN) {
	...
	// Note: DSD does not have Audio Status Channel, so all bytes in the
	// Audio Status Channel registers are zeros. That should not cause
	// problems because the DSD format is fixed.
...
static void switch_audio(bool switch_on)
{
	if (switch_on) {
		...
		SiiRegBitsSet(RX_A__AUDP_MUTE, RX_M__AUDP_MUTE__AUDIO_MUTE,
			      OFF);
	} else {
		if (audio_vars.audio_is_on) {// reduce extra log prints
		...
		// disable I2S output, but keep MCLKenable, SPDIF is on with
		// flat output
		SiiRegWrite(RX_A__AUDRX_CTRL, RX_M__AUDRX_CTRL__PASS_SPDIF_ERR |
...
void RxAudio_Stop(void) { switch_audio(OFF); }
...
{
	bool protected_audio = (acp_GeneralAudio != acp_type);
	...
	//	if((as_AudioOn == audio_vars.rx_audio_state) || (as_AudioReady==
	//audio_vars.rx_audio_state))
	SiiRegBitsSet(RX_A__AUDRX_CTRL, RX_M__AUDRX_CTRL__SPDIF_EN,
...
void RxAudio_Init(void)
{
	...
	audio_vars.fs =
	    AUDIO_CHST4__FS_UNKNOWN; // means "not indicated", default values
				     // for 48kHz audio will be used

	SiiRegWrite(RX_A__TDM_CTRL2,
		    0x01); // configure FS signal width to 1 clock cycle
