...
#define PIXELS_TOLERANCE                                                       \
	30 // should be no more then 55 to distinguish all CEA861C modes
#define LINES_TOLERANCE                                                        \
	15 // should be no more then 29 to distinguish all CEA861C modes

#define H_FREQ_TOLERANCE                                                       \
	2 // H Freq tolerance in kHz, used for analog video detection
...
struct vmd_state_t {
	...
	struct _SiiOsTimerInfo_t *videoStableTimer;
#else
	...
	uint8_t avi_received : 1;
	uint8_t hdmi_3d_vsif_received : 1;
	uint8_t hdmi_vic_received : 1;
};

static struct vmd_state_t vmd_data = {0};

#if defined(__KERNEL__)
static char *str3DStructure[] = {"Frame packing/sequential", "Top-and-Bottom",
				 "Side-by-Side(Half)", "Reserved"};
#endif

uint8_t VMD_GetVideoIndex(void) { return vmd_data.video_index; }

static uint8_t search_861_mode(struct sync_info_type *p_sync_info)
{
	int i;
	...
	for (i = 0; VideoModeTable[i].Vic4x3 || VideoModeTable[i].Vic16x9;
	     i++) {
		const struct videoMode_t *p_video_table = &VideoModeTable[i];
		bool interlaced = p_sync_info->Interlaced;
		uint16_t total_V_lines_measured =
		    (interlaced ? (p_sync_info->TotalLines * 2)
		...
		if (ABS_DIFF(total_V_lines_measured, p_video_table->Total.V) >
		    LINES_TOLERANCE)
			continue;

		// check number of clocks per line (it works for all possible
		// replications)
		if (ABS_DIFF(p_sync_info->ClocksPerLine,
			     p_video_table->Total.H) > PIXELS_TOLERANCE)
			continue;

		// check Pixel Freq (in 10kHz units)
		//		if(ABS_DIFF(p_sync_info->PixelFreq,
		// p_video_table->PixClk) > FPIX_TOLERANCE)  // tolerance based
		// on fixed bandwidth
		if (0 !=
		    ABS_DIFF(
			p_sync_info->PixelFreq,
			p_video_table->PixClk)) // tolerance based on dynamic
						// bandwidth (fixed ratio)
		{
			range = p_video_table->PixClk /
				ABS_DIFF(p_sync_info->PixelFreq,
					 p_video_table->PixClk);
		...
		(int)p_sync_info->ClocksPerLine,
		(int)total_V_lines_measured);

		DEBUG_PRINT(MSG_STAT,
		"clock per lines in video table: %d, lines in  video table: %d\n",
		(int)p_video_table->Total.H,
		(int)p_video_table->Total.V);
#endif

		// if all previous tests passed, then we found at least one mode
		// even polarity is mismatched
		...
		// check polarities
		if ((p_sync_info->HPol == p_video_table->HPol) &&
	...
	case found_not_exact:
		DEBUG_PRINT(MSG_STAT,
			    ("RX: Warning: not exact video mode found\n"));
		break;
	}
...
static void verify_cea861vic(uint8_t vid_idx)
{
	...
	if (vmd_data.avi_received) {
		...
		if ((vic != 0) && // if AVI with VIC has been received
		    (SI_VIDEO_MODE_NON_STD !=
		     vid_idx) // if resolution has been measured
		) {
			uint8_t table_idx =
			    vid_idx & ~SI_VIDEO_MODE_3D_RESOLUTION_MASK;
			bool vic_correct = false;

			if (table_idx < (NMB_OF_CEA861_VIDEO_MODES +
					 NMB_OF_HDMI_VIDEO_MODES)) {
				// verify VIC
				vic_correct =
...
static bool is_video_in_range(struct sync_info_type *p_sync_info)
{
	bool test_passed = false;

	if ((p_sync_info->ClocksPerLine < 100) ||
	    (p_sync_info->TotalLines < 100)) {
		// Also prevents from devision by 0 in h_freq and v_freq
		// calculations.
		DEBUG_PRINT(MSG_STAT,
		"RX: Unsuitable Video: %d clocks per line, %d total lines\n",
		(int)p_sync_info->ClocksPerLine,
		(int)p_sync_info->TotalLines);
	} else {
		uint16_t pix_freq = p_sync_info->PixelFreq; // in 10kHz units
		uint8_t h_freq =
		    ((uint32_t)(((uint32_t)p_sync_info->PixelFreq) * 10 + 5)) /
		    p_sync_info->ClocksPerLine; // in 1 kHz units
		uint16_t v_freq = (((uint32_t)h_freq) * 1000 + 500) /
				  p_sync_info->TotalLines; // in 1 Hz units

		if (pix_freq <= (((uint16_t)VIDEO__MAX_PIX_CLK_10MHZ) * 1000 +
				 FPIX_TOLERANCE)) {
			if ((h_freq + FH_TOLERANCE >= VIDEO__MIN_H_KHZ) &&
			    (h_freq <= (VIDEO__MAX_H_KHZ +
					FH_TOLERANCE))) { // in 1 kHz units
				if ((v_freq + FV_TOLERANCE >=
				     VIDEO__MIN_V_HZ) &&
				    (v_freq <=
				     VIDEO__MAX_V_HZ + FV_TOLERANCE)) {
					test_passed = true;
				}
			}
		}
...
{
#if (SI_USE_DEBUG_PRINT == ENABLE)
	struct vmd_state_t *p_vmd = &vmd_data;

	DEBUG_PRINT(
	    MSG_STAT,
	    "RX: 3D resolution VIC=0x%02X 3d_struct=0x%02X ext_data=0x%02X\n",
	    (int)p_vmd->cea861_vic, (int)p_vmd->hdmi_3d_structure,
	    (int)p_vmd->hdmi_3d_ext_data);
#endif // SI_USE_DEBUG_PRINT
}

#if defined(__KERNEL__)
// applied both for MHL and HDMI.
char *SiiRx3DTypeGet(void)
{
	struct vmd_state_t *p_vmd;
	int index;

	p_vmd = &vmd_data;

	switch (p_vmd->hdmi_3d_structure) {
	case 0:
		index = 0;
		break;
	case 6:
...
static void print_hdmi_vic_resolution(void)
{
	...
	DEBUG_PRINT(MSG_STAT,
		    "RX: HDMI VIC: %d, Idx = %d, %d x %d %c @ %d Hz\n",
		    (int)VideoModeTable[vid_idx].HdmiVic, (int)vid_idx,
		    (int)VideoModeTable[vid_idx].Active.H,
		    (int)VideoModeTable[vid_idx].Active.V,
		    VideoModeTable[vid_idx].Interlaced ? 'I' : 'P',
		    (int)VideoModeTable[vid_idx].VFreq);
...
static void save_sync_info(struct sync_info_type *p_sync_info)
{
	struct SiiRxTiming_t rxTiming;
	...
	rxTiming.hPol = p_sync_info->HPol, rxTiming.vPol = p_sync_info->VPol,
...
// called 3D_Structure. This function corrects the timing according to the
// parameter.
static void
correct_sync_info_according_to_3d_mode(struct sync_info_type *p_sync_info)
{
	...
	if (vmd_data.hdmi_vic_received) {
		...
		if ((hdmi_vic > 0) && (hdmi_vic <= LAST_KNOWN_HDMI_VIC)) {
			detected_video_index =
			    hdmiVicToVideoTableIndex[hdmi_vic];
		}
		...
		if ((cea_vic > 0) && (cea_vic <= LAST_KNOWN_CEA_VIC)) {
			detected_video_index =
			    ceaVicToVideoTableIndex[cea_vic] |
...
// This function takes video timing based information from the video timing
// table and fills p_sync_info structure with this timing information for
// vid_idx video index. Returnstrue on success, false on failure. Note:
// Repetition is not taken in account by this function because the function is
// used for 3D and HDMI VIC (4K2K) formats which do not use repetition by
// HDMI 1.4a specification. Moreover, HDMI 1.4a does not specify how exactly
// treat the formats with repetition. The function might require modification in
// the future if such support is required.
static bool
fill_sync_info_from_video_table(struct sync_info_type *p_sync_info,
				uint8_t vid_idx)
{
	bool success = false;

	if (vid_idx < (NMB_OF_CEA861_VIDEO_MODES + NMB_OF_HDMI_VIDEO_MODES)) {
		const struct videoMode_t *p_video_table =
		 &VideoModeTable[vid_idx];
...
static uint8_t detect_video_resolution(struct sync_info_type *p_sync_info)
{
	...
	if (detected_video_idx == SI_VIDEO_MODE_NON_STD) {
		...
		if (is_video_in_range(p_sync_info)) {
			...
			if (detected_video_idx != SI_VIDEO_MODE_NON_STD) {
				...
				// If it does not match, an error message will
				// be printed. Measured format has higher
				// priority than AVI's VIC.
				verify_cea861vic(detected_video_idx);
			} else {
				// Input does not much to any known CEA861 mode
				// or 3D format. It can be considered as a PC
				// mode if such compilation option is enabled.
#if (SI_ALLOW_PC_MODES == ENABLE)
				// In other words, consider any non- CEA-861D or
				// non-3D format as a PC resolution if video
				// timing parameters are within allowed range.
		...
		// 3D video
		if (fill_sync_info_from_video_table(
			p_sync_info, (detected_video_idx &
				      ~SI_VIDEO_MODE_3D_RESOLUTION_MASK))) {
...
#define MAX_REPORT_DATA_STRING_SIZE 20
	...
	if (vid_idx == SI_VIDEO_MODE_NON_STD)
		scnprintf(str, MAX_REPORT_DATA_STRING_SIZE, "out of range");
	else if (vid_idx == SI_VIDEO_MODE_PC_OTHER)
	...
	send_sii5293_uevent(devinfo->device, DEVICE_EVENT,
			    DEV_INPUT_VIDEO_MODE_EVENT, str);
}
#endif
...
bool VMD_DetectVideoResolution(void)
{
	...
	struct sync_info_type sync_info;
	uint8_t vid_idx;
	...
	if (vmd_data.bReadyforVMD) {
		...
		gDriverContext.avi_vic = vmd_data.cea861_vic;
		...
		if (vid_idx == SI_VIDEO_MODE_NON_STD)
			...
			print_861_resolution(); // CEA-861D resolutions
...
void VMD_VsifProcessing(uint8_t *p_packet, uint8_t length)
{
	...
	if (p_packet[0] == 0x03  &&  p_packet[1] == 0x0C &&
	  p_packet[2] == 0x00) {
	...
	else if (p_packet[0] == 0x1D && p_packet[1] == 0xA6 &&
		  p_packet[2] == 0x7C) {
...
void VMD_HdmiVsifProcessing(uint8_t *p_packet, uint8_t length)
{
	...
	case 2: // 3D format
		...
		if (vmd_data.hdmi_3d_structure == 0)
...
void VMD_MhlVsifProcessing(uint8_t *p_packet, uint8_t length)
{
	// p_packet should point to an HDMI VSIF packet.
	// No other VSIF packets are supported by this function.
	uint8_t mhl_video_format =
	    p_packet[3] & 0x03;	    // get the MHL_VID_FMT field
	uint8_t mhl_3d_format_type; // to keep the MHL_3D_FMT_TYPE field
	bool bValid3DVsif = false;

	// Check whether 3D format present
	if (mhl_video_format != 0x01 || length < 4)
		return;

	mhl_3d_format_type =
	    (p_packet[3] & 0x3F) >> 2; // get the MHL_3D_FMT_TYPE field

	switch (mhl_3d_format_type) {
	case 0: // Frame Sequential 3D Video
		bValid3DVsif = true;
		vmd_data.hdmi_3d_structure =
		    0x00; // Store it for further processing.
	...
	case 1: // Top-Bottom 3D Video
		bValid3DVsif = true;
		vmd_data.hdmi_3d_structure =
		    0x06; // Store it for further processing.
		vmd_data.hdmi_3d_ext_data = 0x00; // Reset to Zero
		break;
	...
	case 2: // Left-Right 3D Video
		bValid3DVsif = true;
		vmd_data.hdmi_3d_structure =
		    0x08; // Store it for further processing.
...
{
	uint32_t packet_analysis = NOT_HDMI_VSIF;

	// DEBUG_PRINT(MSG_ALWAYS, ("VMD_GetVsifPacketType\n"));

	// Check HDMI VSIF signature.
	// HDMI IEEE Registration Identifier is 0x000C03 (least significant byte
	// first).
	if ((p_packet[0] == 0x03) && (p_packet[1] == 0x0C) &&
	    (p_packet[2] == 0x00)) {
	...
	// MHL IEEE Registration Identifier is 0x7CA61D (least significant byte
	// first).
...
uint32_t VMD_GetHdmiVsifPacketType(uint8_t *p_packet, uint8_t length)
{
	uint32_t packet_analysis = NOT_HDMI_VSIF;
	...
	if ((p_packet[0] == 0x03) && (p_packet[1] == 0x0C) &&
		...
		switch (hdmi_video_format) {
		case 0:
			// It just a blank VSIF
			packet_analysis = BLANK_VSIF;
			break;
		case 1:
		...
		case 2:
			...
			if (vmd_data.hdmi_3d_vsif_received) {
				...
				if (vmd_data.hdmi_3d_structure !=
				    p_packet[4] >> 4) {
					// 3D_Structure is different; the packet
					// is new 3D.
					packet_analysis = NEW_3D;
				}
				// Side-by-Side (Half) has at least one
				// additional parameter. Check if it matches to
				// the previously received one.
				if ((vmd_data.hdmi_3d_structure == 8) &&
				    (length > 4)) {
					// 3D_Ext_Data field
					if (vmd_data.hdmi_3d_ext_data !=
					    (p_packet[5] >> 4)) {
						// 3D_Structure is different;
						// the packet is new 3D.
						packet_analysis = NEW_3D;
					}
				}
			} else {
...
uint32_t VMD_GetMhlVsifPacketType(uint8_t *p_packet, uint8_t length)
{
	...
	// MHL IEEE Registration Identifier is 0x7CA61D (least significant byte
	// first).
	if ((p_packet[0] == 0x1D) && (p_packet[1] == 0xA6) &&
	    (p_packet[2] == 0x7C)) {
		// MHL VSIF signature is found.
		// Check MHL format.
		...
		switch (mhl_3d_format_type) {
		case 0: // Frame Sequential 3D Video
			if (vmd_data.hdmi_3d_structure != 0x00 ||
			    vmd_data.hdmi_3d_vsif_received == false) {
				packet_analysis = NEW_3D;
			}
			break;
		...
		case 1: // Top-Bottom 3D Video
			if (vmd_data.hdmi_3d_structure != 0x06)
				packet_analysis = NEW_3D;
			break;

		case 2: // Left-Right 3D Video
			if (vmd_data.hdmi_3d_structure != 0x08)
				packet_analysis = NEW_3D;
			// FD: no need to worry about this per MHL 2.0 spec,
			// there is only one value 0001 for Left-Right 3D Video
			/*
		...
		default:
			DEBUG_PRINT(MSG_ALWAYS,
				    ("Unrecognized MHL 3D format type.\n"));
			break;
		}
	}
...
{
	if (vmd_data.hdmi_3d_vsif_received || vmd_data.hdmi_vic_received) {
		...
		// Changing from 3D/4K2K to non-3D/4K2K in most cases causes
		// pixel clock interruption that change RX state machine's
		// state. In this case no other processing is required. However,
		// some 3D to 2D formats have compatible timings and it is
		// possible skipping such transaction. This function is called
		// when HDMI VSIF packets are no longer coming. That event may
		// happen on 3D to 2D change. The function starts format
		// verification.
		VMD_DetectVideoResolution();
	}
}
...
// data is not valid in 3D/HDMI_VIC mode, it always returns false if 3D/HDMI_VIC
// is detected. PC resolutions always return true.
bool VMD_WasResolutionChanged(void)
{
	...
	if (measured_video_idx == SI_VIDEO_MODE_PC_OTHER) {
		// PC resolutions.
		// Since we do not keep the timing information for PC
		// resolutions, assume that the resolution change event is real.
...
{
#if defined(__KERNEL__)
	gDriverContext.input_video_mode = SI_VIDEO_MODE_NOT_STABLE;
#endif
...
void SiiRxFormatDetect(void)
{
	if (vmd_data.videoStableTime) {
		// Wait 100ms after sync detect to determine the video
		// resolution.
		if (SiiTimerTotalElapsed() - vmd_data.videoStableTime >
		    VIDEO_STABLE_TIME) {
...
void VMD_Init(void)
{
	memset(&vmd_data, 0, sizeof(struct vmd_state_t));
	VMD_ResetTimingData();
#if defined(__KERNEL__)
	SiiOsTimerCreate("Vid Timer", VMD_Timer_Callback, NULL,
			 &vmd_data.videoStableTimer);
#endif
}
...
// or zero otherwise.
uint16_t VMD_GetPixFreq10kHz(void) { return vmd_data.pix_freq; }

void SiiRxSetVideoStableTimer(void)
{
	bool bScdtState = false;
...
#if defined(__KERNEL__)
		if (bScdtState) {
			SiiOsTimerStart(vmd_data.videoStableTimer,
					VIDEO_STABLE_TIME);
		} else {
			SiiDrvSoftwareReset(
			    RX_M__SRST__HDCP_RST); // Do HDCP reset when there's
						   // no scdt
