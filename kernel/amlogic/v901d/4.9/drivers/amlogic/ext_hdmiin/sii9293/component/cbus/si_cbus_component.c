...
struct CbusInstanceData_t cbusInstance;
struct CbusInstanceData_t *pCbus = &cbusInstance;

//------------------------------------------------------------------------------
// Function:    SiiCbusRequestStatus
// Description: Return the status of the message currently in process, if any.
// Returns:     CBUS_REQ_IDLE, CBUS_REQ_PENDING, CBUS_REQ_SENT, or
// CBUS_REQ_RECEIVED
//------------------------------------------------------------------------------
...
uint8_t SiiCbusChannelStatus(void) { return pCbus->chState.state; }
...
{
	memcpy(pCmdRequest, &pCbus->chState.request[CH_ACTIVE_INDEX],
	       sizeof(struct cbus_req_t));
}
...
bool SiiMhlRxCbusConnected(void) { return pCbus->chState.connected; }
...
static int8_t CBusSendNextInQueue(void)
{
	...
	if ((pCbus->chState.request[CH_ACTIVE_INDEX].reqStatus !=
	     CBUS_REQ_PENDING)) {
#ifdef MHAWB_SUPPORT
		if (pCbus->chState.connected &&
		    (!pCbus->chState.burstWaitState)) {
			if (pCbus->chState.remote_dcap[0x0A] &
			    MHL_FEATURE_SP_SUPPORT)
				SiiDrvHawbEnable(
				    true); // There's no pending CBUS message
					   // need to send out, take HAWB ON
		}
#endif
		return SUCCESS;
	}
...
#endif
	{
		...
		if (SiiDrvInternalCBusWriteCommand(
			&pCbus->chState.request[CH_ACTIVE_INDEX])) {
			pCbus->chState.state = CBUS_SENT;
			pCbus->chState.request[CH_ACTIVE_INDEX].reqStatus =
			    CBUS_REQ_SENT;
		} else {
...
//              If SUCCESS, command data is returned in
//              pCbus->chState.msgData[i]
//------------------------------------------------------------------------------
...
static uint8_t CBusProcessSubCommand(uint8_t *pData)
{
	...
	DEBUG_PRINT(MSG_DBG,
		    "CBUS:: MSG_MSC CMD:  0x%02X  MSG_MSC Data: 0x%02X\n",
...
// Description: Set the specified channel state to IDLE. Clears any messages
// that
//              are in progress or queued.  Usually used if a channel connection
//              changed or the channel heartbeat has been lost.
//------------------------------------------------------------------------------

static void CBusResetToIdle(void) { SiiMhlRxInitialize(); }
...
bool SiiMhlRxIsQueueFull(void)
{
	...
	for (queueIndex = 0; queueIndex < CBUS_MAX_COMMAND_QUEUE;
	     queueIndex++) {
		if (pCbus->chState.request[queueIndex].reqStatus ==
		    CBUS_REQ_IDLE) {
			return false;
		}
	}
...
bool SiiCbusWriteCommand(struct cbus_req_t *pReq)
{
	...
	if (SiiMhlRxCbusConnected()) {
		...
		if (SiiCbusAbortStateGet() == false) {
			// DEBUG_PRINT( MSG_DBG, "CBUS:: SiiCbusWriteCommand::
			// Channel State: %02X", (int)pCbus->chState[
			// channel].state );
			for (i = 0; i < CBUS_MAX_COMMAND_QUEUE; i++) {
				queueIndex = (CH_ACTIVE_INDEX + i) %
					     CBUS_MAX_COMMAND_QUEUE;
				if (pCbus->chState.request[queueIndex]
					.reqStatus == CBUS_REQ_IDLE) {
					// Found an idle queue entry, copy the
					// request and set to pending.
					memcpy(
					    &pCbus->chState.request[queueIndex],
					    pReq, sizeof(struct cbus_req_t));
					pCbus->chState.request[queueIndex]
					    .reqStatus = CBUS_REQ_PENDING;
					pCbus->chState.request[queueIndex]
					    .retry = 1;
					success = true;
					break;
...
bool SiiCbusSendMscMsgCmd(uint8_t subCmd, uint8_t mscData)
{
	...
	if (pCbus->chState.state == CBUS_IDLE) {
		CBusSendNextInQueue(); // No command in progress, write new
				       // command immediately.
	}
	return true;
}
...
bool SiiMhlRxSendUcpk(uint8_t keyCode)
{
...
// Description: If any interrupts on the specified channel are set, process
// them. Returns:     SUCCESS or CBUS_SOFTWARE_ERRORS_t error code.
//------------------------------------------------------------------------------
	...
	if (SiiDrvCbusIntrFlagGet()) {
		...
		if (SiiDrvCbusBusStatusGet(&busStatus)) {
			...
			DEBUG_PRINT(MSG_DBG,
				    ...
				    pCbus->chState.connected ? "Connected"
							     : "Disconnected");
			if (pCbus->chState.connected) {
				...
				if (SiiCbusSendDcapRdy()) {
					...
					if (SiiMhlRxSendDcapChange()) {
						// send set_hpd and set path_en
						// bit too
						SiMhlRxHpdSet(true);
					}
				}
#if defined(__KERNEL__)
				SiiOsTimerStart(pCbus->chState.dcapTimer,
						CBUS_DCAP_READY_TIMER);
#else
				pCbus->chState.dcapTimer =
				    SiiTimerTotalElapsed();
#endif
			} else {
		...
		if (SiiDrvCbusReqWrtGet()) {
			DEBUG_PRINT(
			    MSG_DBG,
			    "Grant peer's request to write scratchpad\n");
			SiiCbusGrtWrt();
		}
		...
		if (SiiDrvCbusGrtWrtGet() && pCbus->chState.burstWaitState) {
			...
			SiiCbusWriteBurst();
			// Receive grt, stop burst timer.
			pCbus->chState.burst[CH_ACTIVE_BURST].burstStatus =
			    CBUS_BURST_IDLE;
		...
		if (SiiDrvCbus3DReqGet()) {
			DEBUG_PRINT(MSG_DBG, "Peer request 3D information\n");
		...
		if (SiiDrvCbusMscAbortReasonGet(&temp)) {
			...
			if ((temp & ~(ERROR_CBUS_ABORT | BIT_MSC_MAX_RETRY |
				      BIT_MSC_TIMEOUT)) != 0)
		...
		if (SiiDrvCbusMscAbortResReasonGet(&temp)) {
			DEBUG_PRINT(
			    MSG_DBG,
			    "MSC CMD aborted as Responder , reason: %02X\n",
			    (int)temp);
			if (temp & BIT_MSC_ABORT_BY_PEER) // Abort by peer
				result |= ERROR_CBUS_ABORT;
			else if (temp)
...
#if defined(__KERNEL__)
			SiiOsTimerStart(pCbus->chState.burstTimer,
					CBUS_BURST_WAIT_TIMER);
#else
				   ...
				   uint8_t *pMsgData)
{
	...
	for (i = 0; i < CBUS_MAX_BURST_QUEUE; i++) {
		if (pCbus->chState.burst[i].burstStatus == CBUS_BURST_IDLE) {
			// Found an idle queue entry, copy the request and set
			// to pending.
			memcpy(&pCbus->chState.burst[i].burstData, pMsgData,
			       length);
			pCbus->chState.burst[i].offset = startOffset;
			pCbus->chState.burst[i].length = length;
			pCbus->chState.burst[i].burstStatus =
			    CBUS_BURST_PENDING;
...
bool SiiCbusWriteBurst(void)
{
	...
	memcpy(req.msgData, pCbus->chState.burst[CH_ACTIVE_BURST].burstData,
	       req.length);
	if (!(SiiCbusWriteCommand(&req))) {
...
bool SiiMhlRxReadDevCapReg(uint8_t regOffset)
{
	...
	if (SiiCbusWriteCommand(&req) != success)
...
bool SiiCbusSendMscCommand(uint8_t cmd)
{
	...
	case MHL_GET_SC3_ERRORCODE:
		...
		if (SiiCbusWriteCommand(&req) != success) {
			DEBUG_PRINT(MSG_ERR, "Couldn't send cmd: %02X to peer",
				    (int)cmd);
			return false;
		}
	...
	default:
		DEBUG_PRINT(MSG_ERR, "Invalid command %02X send request!!",
			    (int)cmd);
		success = false;
	}
	return success;
...
//------------------------------------------------------------------------------
bool SiMhlRxSendEdidChange(void) { return SiiCbusSetInt(0x01, BIT1); }

//------------------------------------------------------------------------------
// Function:    SiiMhlRxSendDcapChange
//------------------------------------------------------------------------------
bool SiiMhlRxSendDcapChange(void) { return SiiCbusSetInt(0x00, BIT0); }

//------------------------------------------------------------------------------
// Function:    SiiDscrChange
//------------------------------------------------------------------------------
bool SiiCbusSendDscrChange(void) { return SiiCbusSetInt(0x00, BIT1); }

//------------------------------------------------------------------------------
// Function:    SiiReqWrt
//------------------------------------------------------------------------------
bool SiiCbusReqWrt(void) { return SiiCbusSetInt(0x00, BIT2); }

//------------------------------------------------------------------------------
// Function:    SiiGrtWrt
// Description:
//------------------------------------------------------------------------------
bool SiiCbusGrtWrt(void) { return SiiCbusSetInt(0x00, BIT3); }
...
{
	if (enable) {
		// enable PATH_EN bit on peer's appropriate status register
		// (offset 0x31)
		DEBUG_PRINT(MSG_STAT, "SiiMhlRxPathEnable:: Enable\n");
		return SiiCbusWriteStatus(0x01, BIT3);
	}

	// disable PATH_EN bit on peer's appropriate status register
	// (offset 0x31)
...
//------------------------------------------------------------------------------
bool SiiCbusSendDcapRdy(void) { return SiiCbusWriteStatus(0x00, BIT0); }
...
static void CbusDcap_Timer_Callback(void *pArg)
{
	if (pCbus->chState.connected && (!pCbus->chState.dcap_ready)) {
		...
		pCbus->chState.dcap_ongoing = true;
		if (pCbus->chState.state == CBUS_IDLE) {
			CBusSendNextInQueue(); // No command in progress, write
...
{
	if (pCbus->chState.abortState &&
	    (SiiTimerTotalElapsed() - pCbus->chState.abortTimer >
	     CBUS_ABORT_TIMER)) {
		DEBUG_PRINT(MSG_ALWAYS, ("CBUS Abort timer expired.\n"));
		pCbus->chState.abortState = false;
	}
	if (pCbus->chState.burstWaitState &&
	    (SiiTimerTotalElapsed() - pCbus->chState.burstTimer >
	     CBUS_BURST_WAIT_TIMER)) {
	...
	if (pCbus->chState.connected && (!pCbus->chState.dcap_ready) &&
	    (SiiTimerTotalElapsed() - pCbus->chState.dcapTimer >
	     CBUS_DCAP_READY_TIMER)) {
		DEBUG_PRINT(MSG_ERR, ("CBUS Dcap ready timer expired.\n"));
		pCbus->chState.dcap_ready = true;
		// Read Dcap
...
// Description: Check the state of any current CBUS message on specified
// channel.
...
uint8_t SiiMhlRxIntrHandler(void)
{
	...
	if (result != SUCCESS) {
		if (result & ERROR_CBUS_ABORT) {
			...
			if (pCbus->chState.state == CBUS_SENT) {
			   ...
			   (ERROR_CBUS_TIMEOUT | ERROR_NACK_FROM_PEER)) {
	...
	if (pCbus->chState.receive.arrived) {
		SiiMhlRxRcpRapRcvdNtfy(pCbus->chState.receive.command,
				       pCbus->chState.receive.offsetData);
	...
	if (pCbus->chState.state == CBUS_XFR_DONE) {
		...
		if (pCbus->chState.request[CH_ACTIVE_INDEX].command ==
		    MHL_READ_DEVCAP) {
			DEBUG_PRINT(
			    MSG_DBG, "Response data Received, %02X\n",
			    pCbus->chState.request[CH_ACTIVE_INDEX].retData[0]);
			if (pCbus->chState.request[CH_ACTIVE_INDEX].offsetData <
			    15)
				SiiMhlRxReadDevCapReg(
				    pCbus->chState.request[CH_ACTIVE_INDEX]
					.offsetData +
				    1);
			else
			...
			pCbus->chState
			    .remote_dcap[pCbus->chState.request[CH_ACTIVE_INDEX]
					     .offsetData] =
			    pCbus->chState.request[CH_ACTIVE_INDEX].retData[0];
		}
		if ((pCbus->chState.request[CH_ACTIVE_INDEX].command ==
		     MHL_MSC_MSG) &&
		    ((pCbus->chState.request[CH_ACTIVE_INDEX].msgData[0] ==
		      MHL_MSC_MSG_RCPE) ||
		     (pCbus->chState.request[CH_ACTIVE_INDEX].msgData[0] ==
		      MHL_MSC_MSG_UCPE))) {
		...
		pCbus->chState.request[CH_ACTIVE_INDEX].reqStatus =
		    CBUS_REQ_IDLE;
		CH_ACTIVE_INDEX =
		    (CH_ACTIVE_INDEX + 1) % CBUS_MAX_COMMAND_QUEUE;
	}
#if 0
	if (pCbus->chState.state == CBUS_SENT) {
		...
		if (pCbus->chState.request[CH_ACTIVE_INDEX].reqStatus ==
		    CBUS_REQ_SENT) {
			if (SiiTimerTotalElapsed() -
				pCbus->chState.request[CH_ACTIVE_INDEX]
				    .reqTimer >
			    CBUS_CMD_TIMEOUT) {
				// should never be here, when this timeout
				// occurred, there should be abort interrupt
				// firstly.
				retrycheck = true;
			}
		} else {
	...
	if (pCbus->chState.state == CBUS_IDLE) {
		result = CBusSendNextInQueue(); // No command in progress, write
						// new command immediately.
	}
...
{
	memset(pCbus, 0, sizeof(struct CbusInstanceData_t));
#if defined(__KERNEL__)
	SiiOsTimerCreate("Abt Timer", CbusAbort_Timer_Callback, NULL,
			 &pCbus->chState.abortTimer);
	SiiOsTimerCreate("Bst Timer", CbusBurst_Timer_Callback, NULL,
			 &pCbus->chState.burstTimer);
	SiiOsTimerCreate("Dcap Timer", CbusDcap_Timer_Callback, NULL,
			 &pCbus->chState.dcapTimer);
#endif
...
//------------------------------------------------------------------------------
void SiiCbusAbortStateSet(bool value) { pCbus->chState.abortState = value; }

//------------------------------------------------------------------------------
// Function:    SiiCbusAbortStateGet
//------------------------------------------------------------------------------
bool SiiCbusAbortStateGet(void) { return pCbus->chState.abortState; }
