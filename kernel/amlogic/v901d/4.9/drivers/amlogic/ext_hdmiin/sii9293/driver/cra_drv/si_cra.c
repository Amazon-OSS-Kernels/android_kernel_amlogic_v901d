...
struct CraInstanceData_t craInstance = {
	0,		 // structVersion
	0,		 // instanceIndex
...
// Translate uint32_t return codes into uint32_t status
// codes.
static uint32_t
TranslatePlatformStatus(uint32_t platformStatus)
{
				 ...
				 uint16_t count)
{
	...
	uint32_t platformStatus;
	uint32_t status = SII_ERR_FAIL;
	int retryTimes = 3;
...
uint32_t CraReadIOExpanderI2c(uint32_t busIndex,
				      ...
				      uint16_t count)
{
	struct SiiI2cMsg_t msgs[1];
	uint32_t platformStatus;
	uint32_t status = SII_ERR_FAIL;

	msgs[0].addr = deviceId;
	msgs[0].cmdFlags = SII_MI2C_RD;
	msgs[0].len = count;
	msgs[0].pBuf = (uint8_t *)pBuffer; // cast gets rid of const warning

	platformStatus = SiiMasterI2cTransfer(busIndex, msgs, 1);
	status = TranslatePlatformStatus(platformStatus);

	//    DEBUG_PRINT( MSG_ALWAYS, "CraWriteIOExpanderI2c, status is %d 1st
	//    data=0x%x 2nd data=0x%x\n", (int)status, (int)pBuffer[0],
	//    (int)pBuffer[1]);
...
static uint32_t CraReadBlockI2c(int8_t busIndex, uint8_t deviceId,
					uint8_t regAddr, uint8_t *pBuffer,
					uint16_t count)
{
	uint32_t status = SII_ERR_FAIL;
	...
	do {
		...
		if (I2cSendStart(busIndex, deviceId, &regAddr, 1, FALSE) !=
		    PLATFORM_SUCCESS) {
			break;
		}
		// Receive the requested number of data bytes
		if (I2cReceiveStart(busIndex, deviceId, pBuffer, count, TRUE) !=
		    PLATFORM_SUCCESS) {
			break;
		}
...
static uint32_t CraWriteBlockI2c(int8_t busIndex, uint8_t deviceId,
					 uint8_t regAddr,
					 const uint8_t *pBuffer, uint16_t count)
{
	...
	do {
		...
		if (I2cSendContinue(busIndex, pBuffer, count, TRUE) !=
		    PLATFORM_SUCCESS) {
			break;
		}
		status = SII_SUCCESS;
...
bool SiiCraInitialize(void)
{
	...
	while (g_siiRegPageBaseReassign[i] != 0xFFFF) {
		...
		if ((index < SII_CRA_DEVICE_PAGE_COUNT) &&
		    (g_siiRegPageBaseRegs[index] != 0xFF)) {
...
// Description: Sets the instance for subsequent register accesses.  The
// register
//              access functions use this value as an instance index of the
//              multi- dimensional virtual address lookup table.
// Parameters:  newInstance - new value for instance axis of virtual address
// table. Returns:     None
//------------------------------------------------------------------------------
...
bool SiiRegInstanceSet(SiiReg_t virtualAddress, uint8_t newInstance)
{
	...
	craInstance.lastResultCode = RESULT_CRA_SUCCESS;
	if ((va < SII_CRA_DEVICE_PAGE_COUNT) &&
	    (newInstance < SII_CRA_MAX_DEVICE_INSTANCES)) {
...
void SiiRegReadBlock(SiiReg_t virtualAddr, uint8_t *pBuffer, uint16_t count)
{
	...
	case DEV_I2C_0:
		status = CraReadBlockI2c(DEV_I2C_0, (uint8_t)pPage->address,
					 regOffset, pBuffer, count);
		break;
	case DEV_I2C_2:
		status = CraReadBlockI2c(DEV_I2C_2, (uint8_t)pPage->address,
					 regOffset, pBuffer, count);
		break;
	case DEV_I2C_0_OFFSET:
		status = CraReadBlockI2c(
		    DEV_I2C_0, (uint8_t)pPage->address,
		    regOffset + (uint8_t)(pPage->address >> 8), pBuffer, count);
		break;
	case DEV_I2C_2_OFFSET:
		status = CraReadBlockI2c(
		    DEV_I2C_2, (uint8_t)pPage->address,
		    regOffset + (uint8_t)(pPage->address >> 8), pBuffer, count);
		break;
		      ...
		      uint16_t count)
{
	...
	switch (pPage->busType) {
	case DEV_I2C_0:
		status = CraWriteBlockI2c(DEV_I2C_0, (uint8_t)pPage->address,
					  regOffset, pBuffer, count);
		break;
	case DEV_I2C_2:
		status = CraWriteBlockI2c(DEV_I2C_2, (uint8_t)pPage->address,
					  regOffset, pBuffer, count);
		break;
	case DEV_I2C_0_OFFSET:
		status = CraWriteBlockI2c(
		    DEV_I2C_0, (uint8_t)pPage->address,
		    regOffset + (uint8_t)(pPage->address >> 8), pBuffer, count);
		break;
	case DEV_I2C_2_OFFSET:
