...
struct CraInstanceData_t {
	int structVersion;
	int instanceIndex;
	uint32_t lastResultCode; // Contains the result of the last API
					 // function called
	uint16_t statusFlags;
};

extern struct CraInstanceData_t craInstance;

#ifdef __KERNEL__

/**
 * Status return values returned by platform specific CRA layer support
 * functions.
 */
enum {
	PLATFORM_SUCCESS,
...
// set for ten bit chip address; cleared otherwise
#define SII_MI2C_TEN  0x0010
// set for read data operation; cleared for write operations
#define SII_MI2C_RD 0x0001
// append the buffer from the next message to this message
#define SII_MI2C_APPEND_NEXT_MSG 0x0002
	uint16_t len;  //buffer length
//pointer to input (for write operations) or output (for read operations) buffer
	uint8_t *pBuf;
};
// Maximum # of bytes in a single I2c transfer
#define MAX_I2C_TRANSFER 255
// Maximum # of I2c message segments supported by SiiMasterI2cTransfer
 ...
 *
 *  @param[in]		busIndex Selects the physical I2C bus to be
 *used. Can be ignored if only one I2C bus in system.
 *  @param[in]		pMsgs Array of I2C messages to be
 *sent..
