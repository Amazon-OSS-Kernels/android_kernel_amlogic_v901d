...
struct rx_isr_type {
	// uint8_t check_hdcp_on_vsync;
	uint8_t hdcp_fail_cntr;
	uint8_t shadow_interrupt_mask[NMB_OF_RX_INTERRUPTS];
	bool bVidStableChgEvent;
	bool bScdtState;
	bool bCableChgEvent;
	bool bCableState;
};

static struct rx_isr_type rx_isr = {0};
...
ROM const uint8_t default_interrupt_masks[NMB_OF_RX_INTERRUPTS] = {
	...
	RX_M__INTR1__AUTH_DONE | 0,

	// Interrupt 2
	RX_M__INTR2__HDMI_MODE | RX_M__INTR2__VSYNC |
	...
	RX_M__INTR2__VID_CLK_CHANGED | 0,

	// Interrupt 3
	// RX_M__INTR3__NEW_CP_PACKET | //for Deep Color (called on every packet
	// not on the packet change) RX_M__INTR3__CP_SET_MUTE |    // cannot use
	// since continues coming when AVMUTE RX_M__INTR3__PARITY_ERR |
	// RX_M__INTR3__NEW_MPEG_PACKET |
	RX_M__INTR3__NEW_AUD_PACKET |
	// RX_M__INTR3__NEW_SP_PACKET |
	RX_M__INTR3__NEW_AVI_PACKET | 0,
	...
	// RX_M__INTR4__FIFO_OVERRUN |
	// RX_M__INTR4__FIFO_UNDERRUN |
	0,

	// Interrupt 5
	// RX_M__INTR5__FN_CHANGED |
	// RX_M__INTR5__AAC_DONE |
	// RX_M__INTR5__AUDIO_LINK_ERROR |
	// RX_M__INTR5__V_RES_CHANGE |   //There should vid_clk change when
	// resolution change, RX_M__INTR5__H_RES_CHANGE |
	// RX_M__INTR5__POLARITY_CHANGE |
	// RX_M__INTR5__INTERLACED_CHANGED |
	RX_M__INTR5__AUDIO_FS_CHANGED | 0,

	// Interrupt 6
	//this interrupt is enabled to just clear-up the error flag
	// RX_M__INTR6__DC_ERROR |
	// RX_M__INTR6__AUD_FLAT |
	// RX_M__INTR6__CHST_READY | // cannot use this interrupt because it is
	// triggered on every CHST receiving even nothing changes
	...
	// Interrupt 7
	RX_M__INTR7__NO_VSI_PACKET | RX_M__INTR7__NEW_VSI_PACKET |
...
void RxIsr_Init(void)
{
	...
	memcpy(rx_isr.shadow_interrupt_mask, default_interrupt_masks,
	       NMB_OF_RX_INTERRUPTS);

	SiiRegWriteBlock(RX_A__INTR1_MASK, &rx_isr.shadow_interrupt_mask[INT1],
			 4);
	SiiRegWriteBlock(RX_A__INTR5_MASK, &rx_isr.shadow_interrupt_mask[INT5],
			 2);
	SiiRegWriteBlock(RX_A__INTR7_MASK, &rx_isr.shadow_interrupt_mask[INT7],
			 2);
}
 ...
 *
 *****************************************************************************/
void RxIsr_SwitchRxHdcpInterrupts(bool switch_on)
{
	if (switch_on) {
		rx_isr.shadow_interrupt_mask[INT1] |=
		    (RX_M__INTR1__AUTH_START | RX_M__INTR1__AUTH_DONE);

		SiiRegWrite(RX_A__INTR1,
			    RX_M__INTR1__AUTH_START |
				RX_M__INTR1__AUTH_DONE); // interrupt reset
	} else {
		rx_isr.shadow_interrupt_mask[INT1] &= ~RX_M__INTR1__AUTH_START;
		rx_isr.shadow_interrupt_mask[INT1] |= RX_M__INTR1__AUTH_DONE;
	}
	SiiRegWrite(RX_A__INTR1_MASK,
		    rx_isr.shadow_interrupt_mask[INT1]); // set mask
 ...
 *
 *****************************************************************************/
static void switch_NoAVI_interrupt(bool switch_on)
{
	uint8_t pipe = SiiDrvRxInstanceGet();
	uint8_t mask4 = rx_isr[pipe].shadow_interrupt_mask[INT4];

	if (switch_on) {
		SiiRegWrite(RX_A__INTR4,
			    RX_M__INTR4__NO_AVI); // clear No AVI interrupt if
						  // it was raised
		rx_isr[pipe].shadow_interrupt_mask[INT4] |=
		    RX_M__INTR4_MASK__NO_AVI;
	} else {
		rx_isr[pipe].shadow_interrupt_mask[INT4] &=
		    ~RX_M__INTR4_MASK__NO_AVI;
	}
	if (mask4 != rx_isr[pipe].shadow_interrupt_mask[INT4])
		SiiRegWrite(RX_A__INTR4_MASK,
			    rx_isr[pipe].shadow_interrupt_mask[INT4]);
 ...
 *
 *****************************************************************************/
static void switch_NoVSI_interrupt(bool switch_on)
{
	uint8_t pipe = SiiDrvRxInstanceGet();
	...
	if (switch_on) {
		SiiRegWrite(
		    RX_A__INTR7,
		    RX_M__INTR7__NO_VSI_PACKET); // clear No AVI interrupt if it
						 // was raised
		rx_isr[pipe].shadow_interrupt_mask[INT7] |=
		    RX_M__INTR7_MASK__NO_VSI_PACKET;
	} else {
		rx_isr[pipe].shadow_interrupt_mask[INT7] &=
		    ~RX_M__INTR7_MASK__NO_VSI_PACKET;
	}
	if (mask7 != rx_isr[pipe].shadow_interrupt_mask[INT7])
		SiiRegWrite(RX_A__INTR7_MASK,
			    rx_isr[pipe].shadow_interrupt_mask[INT7]);
...
/*
 * @brief	Switch Receive Audio InfoFrame On Every Packet Interrupts
 * Handler
 *
 * @param[in]		switch_on true to switch on; false to
 * switch off
 */
void RxIsr_SwitchReceiveInfoFrameOnEveryPacket(uint8_t info_type,
					       bool switch_on)
{
	switch (info_type) {
	case INFO_AVI:
		SiiRegBitsSet(RX_A__INT_IF_CTRL, RX_M__INT_IF_CTRL__NEW_AVI,
			      switch_on);
		SiiRegWrite(RX_A__INTR3,
			    RX_M__INTR3__NEW_AVI_PACKET); // reset the interrupt
		break;
	case INFO_AUD:
		SiiRegBitsSet(RX_A__INT_IF_CTRL, RX_M__INT_IF_CTRL__NEW_AUD,
			      switch_on);
		SiiRegWrite(RX_A__INTR3,
			    RX_M__INTR3__NEW_AUD_PACKET); // reset the interrupt
		break;
	case INFO_ACP:
		SiiRegBitsSet(RX_A__INT_IF_CTRL, RX_M__INT_IF_CTRL__NEW_ACP,
			      switch_on);
		SiiRegWrite(RX_A__INTR6,
			    RX_M__INTR6__NEW_ACP_PACKET); // reset the interrupt
		break;
	case INFO_VSI:
		SiiRegBitsSet(RX_A__INT_IF_CTRL, RX_M__INT_IF_CTRL__NEW_VSI,
			      switch_on);
		SiiRegWrite(RX_A__INTR7,
			    RX_M__INTR7__NEW_VSI_PACKET); // reset the interrupt
		break;
	default:
		break;
	}
...
static void RxIsr_HdmiDviTransition(void)
{
	if (SiiDrvRxHdmiModeGet()) {
		...
		// It'll help avoiding a false HDCP Error interrupt caused by
		// pre-HDMI counter content. Capture and clear BCH T4 errors.
		SiiRegWrite(RX_A__ECC_CTRL, RX_M__ECC_CTRL__CAPTURE_CNT);
		SiiRegWrite(
		    RX_A__INTR4,
		    RX_M__INTR4__HDCP); // reset the HDCP BCH error interrupt
 ...
 *
 *****************************************************************************/
static void switch_hdcp_failure_check_with_v_sync_rate(bool switch_on)
{
	if (switch_on) {
		rx_isr.hdcp_fail_cntr = 1;
		// rx_isr.check_hdcp_on_vsync = true; // don't clear HDCP
		// Failure Int if this bit is set
	...
	// Clear BCH counter.
	// The counter accomulates BCH errors and  if it is not cleared it can
	// cause an HDCP failure interrupt Capture and clear BCH T4 errors.
	SiiRegWrite(RX_A__ECC_CTRL, RX_M__ECC_CTRL__CAPTURE_CNT);
}
//#ifdef ENABLE_HDCP
/*****************************************************************************/
 ...
 *
 *****************************************************************************/
static void hdcp_error_handler(bool v_sync_mode)
{
	if (v_sync_mode) {
		if (SiiRegReadWord(RX_A__HDCP_ERR) == 0) {
			...
			if (rx_isr.hdcp_fail_cntr == HDCP_FAIL_THRESHOLD_1ST) {
				DEBUG_PRINT(MSG_STAT,
					    ("RX: Cont. BCH Error ***\n"));

				// Reset Ri to notify an upstream device about
				// the failure. In most cases Ri is already
				// mismatched if we see BCH errors, but there is
				// one rare case when the resetting can help. It
				// is when Ri and Ri' are matched all the time
				// but Ri and Ri' switching happens not
				// synchronously causing a snow screen flashing
				// every 2 seconds. It may happen with some old
				// incomplaint sources or sinks (especially
				// DVI).
				SiiRegWrite(RX_A__HDCP_DEBUG,
					    RX_M__HDCP_DEBUG__CLEAR_RI);

				// Clear BCH counter.
				// The counter accumulates BCH errors and
				// if it is not cleared it can cause an HDCP
				// failure interrupt. Capture and clear BCH T4
				// errors.
				SiiRegWrite(RX_A__ECC_CTRL,
					    RX_M__ECC_CTRL__CAPTURE_CNT);

				// repeat HPD cycle if HDCP is not recovered
				// in some time
				rx_isr.hdcp_fail_cntr =
				    HDCP_FAIL_THRESHOLD_1ST -
				    HDCP_FAIL_THRESHOLD_CONTINUED;
			}
		}
	} else {
...
void SiiRxInterruptHandler(void)
{
	...
	DEBUG_PRINT(MSG_DBG,
		"SiiRxInterruptHandler: after shadow interrupts[INT1]:0x%x\n",
		 interrupts[INT1]);
	DEBUG_PRINT(MSG_DBG,
		"SiiRxInterruptHandler: after shadow interrupts[INT2]:0x%x\n",
		interrupts[INT2]);
	DEBUG_PRINT(MSG_DBG,
		"SiiRxInterruptHandler: after shadow interrupts[INT3]:0x%x\n",
		interrupts[INT3]);
	DEBUG_PRINT(MSG_DBG,
		"SiiRxInterruptHandler: after shadow interrupts[INT4]:0x%x\n",
		interrupts[INT4]);
	DEBUG_PRINT(MSG_DBG,
		"SiiRxInterruptHandler: after shadow interrupts[INT5]:0x%x\n",
		interrupts[INT5]);
	DEBUG_PRINT(MSG_DBG,
		"SiiRxInterruptHandler: after shadow interrupts[INT6]:0x%x\n",
		interrupts[INT6]);
	DEBUG_PRINT(MSG_DBG,
		"SiiRxInterruptHandler: after shadow interrupts[INT7]:0x%x\n",
		interrupts[INT7]);
	DEBUG_PRINT(MSG_DBG,
		"SiiRxInterruptHandler: after shadow interrupts[INT8]:0x%x\n",
		interrupts[INT8]);
	...
	if (interrupts[INT2] & RX_M__INTR2__SCDT) {
		DEBUG_PRINT(MSG_STAT, ("RX: Sync detect change\n"));
		switch_hdcp_failure_check_with_v_sync_rate(OFF);
	...
	if ((interrupts[INT5] & RX_M__INTR5__AUDIO_FS_CHANGED) ||
	    (interrupts[INT6] & RX_M__INTR6__CHST_READY)) {
	...
	if (interrupts[INT7] & RX_M__INTR7__NO_VSI_PACKET) {
		// Clear also vsif_received flag (indicating any VSIF packet
		// detection). If there is any other VSIF packet, the flag will
		// be set again shortly.
		RxInfo_NoVsiHandler();
	}
	...
	if (interrupts[INT6] & RX_M__INTR6__CABLE_UNPLUG) {
		if (SiiRegRead(RX_A__INTR6) & RX_M__INTR6__CABLE_UNPLUG) {
			...
			rx_isr.shadow_interrupt_mask[INT6] &=
			    ~RX_M__INTR6__CABLE_UNPLUG; // Disable 5v plug-out
							// interrupt
			rx_isr.shadow_interrupt_mask[INT8] |=
			    RX_M__INTR8__CABLE_IN; // Enable 5v plug-in
						   // interrupt
			SiiRegWrite(RX_A__INTR6_MASK,
				    rx_isr.shadow_interrupt_mask[INT6]);
			SiiRegWrite(RX_A__INTR8_MASK,
				    rx_isr.shadow_interrupt_mask[INT8]);
			DEBUG_PRINT(MSG_STAT,
					"RX INT: INT6 out:0x%x  INT8 in:0x%x\n",
					rx_isr.shadow_interrupt_mask[INT6],
					rx_isr.shadow_interrupt_mask[INT8]);
		}
	}
	if (interrupts[INT8] & RX_M__INTR8__CABLE_IN) {
		if (SiiRegRead(RX_A__INTR8) & RX_M__INTR8__CABLE_IN) {
			...
			rx_isr.shadow_interrupt_mask[INT6] |=
			    RX_M__INTR6__CABLE_UNPLUG; // Enable 5v plug-out
						       // interrupt
			rx_isr.shadow_interrupt_mask[INT8] &=
			    ~RX_M__INTR8__CABLE_IN; // Disable 5v plug-in
						    // interrupt
			SiiRegWrite(RX_A__INTR6_MASK,
				    rx_isr.shadow_interrupt_mask[INT6]);
			SiiRegWrite(RX_A__INTR8_MASK,
