...
static struct i2c_dev_info device_addresses[] = {
	I2C_DEV_INFO(DEV_PAGE_PP_0), I2C_DEV_INFO(DEV_PAGE_PP_1),
	I2C_DEV_INFO(DEV_PAGE_PP_2), I2C_DEV_INFO(DEV_PAGE_PP_9),
...
static int32_t MhlI2cProbe(struct i2c_client *client,
 ...
 *	 structure of the matching I2C device.
 *	@param[in]		i2c_device_id	Index within
 *	MhlI2cIdTable of the matching I2C device.
 *
 ...
 *
 *  Remove would be called if the I2C device were removed from the system (very
 *unlikley).
 *
 *  All we need to do is clear our copy of the i2c_client pointer to indicate we
 *no longer have an I2C device to work with.
 *
 *  @param[in]	client	Pointer to the client structure representing the I2C
 ...
 *
 * @param DeviceName, the name  must be same as which is registered in your
 * kernel code; we use probe I2C mode , so
 *
...
uint32_t HalOpenI2cDevice(char const *DeviceName, char const *DriverName)
{
	...
	gMhlDevice.pI2cAdapter = i2c_get_adapter(1);
	if (gMhlDevice.pI2cAdapter == NULL) {
		pr_err("%s() failed to get i2c adapter i2c_bus=%d\n",
			__func__,
			1);
		goto done;
	}
	...
	for (idx = 0; idx < ARRAY_SIZE(device_addresses); idx++) {
		if (idx == 0) {
			pr_err("%s() index=0 register i2c_new_device\n",
				__func__);
			gMhlDevice.pI2cClient =
				i2c_new_device(gMhlDevice.pI2cAdapter,
						&si_5293_i2c_boardinfo[idx]);
			device_addresses[idx].client = gMhlDevice.pI2cClient;
		} else {
			pr_err("%s() idex=%d register i2c_new_device\n",
				__func__,
				idx);
			device_addresses[idx].client =
			    i2c_new_dummy(gMhlDevice.pI2cAdapter,
					  device_addresses[idx].dev_addr);
		}
		if (device_addresses[idx].client == NULL) {
			pr_err(
			    "%s() idex=%d register i2c_new_device failed!!!\n",
			    __func__, idx);
			goto err_exit;
		}
	}
...
uint8_t I2C_ReadBlock(uint8_t deviceID, uint8_t offset, uint8_t *buf,
		      uint8_t len)
{
	struct SiiI2cMsg_t msgs[2];
	uint32_t platformStatus = PLATFORM_SUCCESS;
	int retryTimes = 3;
 ...
 *
 *****************************************************************************/
uint8_t I2C_WriteBlock(uint8_t deviceID, uint8_t offset, uint8_t *buf,
		       uint8_t len)
{
	struct SiiI2cMsg_t msgs[2];
	uint32_t platformStatus = PLATFORM_SUCCESS;
 ...
 ****************************************************************************
 */
uint32_t SiiMasterI2cTransfer(uint32_t busIndex,
	struct SiiI2cMsg_t *pMsgs, uint8_t msgNum)
{
	uint8_t	idx;
	struct i2c_msg i2cMsg[MAX_I2C_MESSAGES];
	uint8_t	*pBuffer = NULL;
	uint32_t siiStatus = PLATFORM_FAIL;
	int i2cStatus;
	uint8_t msgCount = 0;
	...
	do {
		...
		if (busIndex != DEV_I2C_1) {//dennis
			SII_DEBUG_PRINT(MSG_ERR,
			"SiiMasterI2cTransfer error: only one I2C bus\n");
			break;
		}

		if (msgNum > MAX_I2C_MESSAGES) {
			SII_DEBUG_PRINT(MSG_ERR,
			"SiiMasterI2cTransfer error: only %d message segments\n",
			MAX_I2C_MESSAGES);
			break;
		}

		// Function parameter checks passed,
		// assume at this point that the
		...
		for (idx = 0; idx < msgNum; idx++) {
			...
			i2cMsg[idx].flags	= (pMsgs[idx].cmdFlags
								& SII_MI2C_RD)
								? I2C_M_RD : 0;
			...
			if (pMsgs[idx].cmdFlags & SII_MI2C_APPEND_NEXT_MSG) {
				// Caller is asking that we append
				// the buffer from the next
				// message to this one.  We will do
				// this IF there is a next
				// message AND the direction of the
				// two messages is the same
				// AND we haven't already appended
				// a message.

				siiStatus = PLATFORM_INVALID_PARAMETER;
				if (idx+1 < msgNum && pBuffer == NULL) {
					if (!((pMsgs[idx].cmdFlags ^
						pMsgs[idx+1].cmdFlags)
						& SII_MI2C_RD)) {
						i2cMsg[idx].len +=
						pMsgs[idx+1].len;
						pBuffer = kmalloc(
		...
		i2cStatus = i2c_transfer(gMhlDevice.pI2cClient->adapter,
							i2cMsg,
							msgCount);
//Xin Commented out here		SII_DEBUG_PRINT(MSG_DBG,
//		"i2c_transfer rw=%d msgCount=%d deviceId: 0x%02x regOffset: 0x%02x\n",
//		msgCount,
//		(pMsgs[i2cStatus].cmdFlags & SII_MI2C_RD) ? I2C_M_RD : 0,
//		pMsgs->addr, *pMsgs->pBuf);
		...
		if (i2cStatus < msgCount) {
			// All the messages were not
			// transferred, some sort of error occurred.
			// Try to return the most appropriate
...
//				*pMsgs->pBuf);
				...
				// One or more messages transferred so error
				// probably occurred on the
				// first unsent message.  Look to see if the
				// message was a read or write
