...
/*****************************************************************************/
/**
 *  @brief		This is a notification API for Cbus connection change,
 *prototype is defined in si_cbus_component.h
 *
 *****************************************************************************/
void SiiMhlRxConnNtfy(bool connected)
{
	...
	send_sii5293_uevent(
	    devinfo->mhl->device, MHL_EVENT,
	    connected ? MHL_CONNECTED_EVENT : MHL_DISCONNECTED_EVENT, NULL);
...
/*****************************************************************************/
/**
 *  @brief This is a notification API for scratchpad bein written by
 *peer
 *
 *****************************************************************************/
...
#define MAX_MHL_CHAR_REPORT_DATA_STRING_SIZE 20
	...
	scnprintf(char_report_data_str, MAX_MHL_CHAR_REPORT_DATA_STRING_SIZE,
		  "0x%02X", rcvdCode);
#endif
	// DEBUG_PRINT( MSG_DBG, "\nApplication layer:: SiiMhlRxRcpRapRcvdNtfy()
	// called !!\n" );
	switch (cmd) {
	...
	case MHL_MSC_MSG_RCPK:
		...
		if (gDriverContext.rcp_out_keycode == rcvdCode) {
			sysfs_notify(&devinfo->mhl->device->kobj, "rcp",
				     "out_status");
			send_sii5293_uevent(devinfo->mhl->device, MHL_EVENT,
					    MHL_RCP_ACKED_EVENT,
					    char_report_data_str);
	...
	case MHL_MSC_MSG_UCPK:
		...
		if (gDriverContext.ucp_out_keycode == rcvdCode) {
			sysfs_notify(&devinfo->mhl->device->kobj, "ucp",
				     "out_status");
			send_sii5293_uevent(devinfo->mhl->device, MHL_EVENT,
					    MHL_UCP_ACKED_EVENT,
					    char_report_data_str);
