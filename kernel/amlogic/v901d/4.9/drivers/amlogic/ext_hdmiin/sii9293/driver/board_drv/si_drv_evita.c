/*
 * drivers/amlogic/ext_hdmiin/sii9293/driver/board_drv/si_drv_evita.c
 *
...
bool SiiDrvEvitaInit(void)
{
	...
	// HDMI Mode
	uint32_t inVideoPath = SiiDrvRxGetOutVideoPath();
	uint8_t csType;
	bool success = true;
	...
	SiiRegWrite(PP_PAGE_4 | 0x3E,
		    0x33); // Enable the ,AVI,AUD infoFrame Packet each frame
	SiiRegWrite(
	    PP_PAGE_4 | 0x3F,
	    0x0C); // Enable the ,General CTL infoFrame Packet each frame
	...
	// Set Evita's output to RGB as default
	...
	SiiRegWrite(PP_PAGE_3 | 0x09, csType);
...
void SiiDrvEvitaAviIfUpdate(void)
{
	...
	uint32_t csType;
	uint8_t ifData[17];
	bool YcMuxFlag = false;

	// When sending AVI infoframe, disable CSC in the Evita
	SiiRegWrite(PP_PAGE_3 | 0x09, 00);
	if (inVideoPath == PATH__RGB)
	...
	if (csType == ColorSpace_RGB) {
		ifData[6] =
		    0x08; // 0x08: full range, 0x04: limited range, only for RGB
	} else {
		ifData[6] = 0;
	}
	ifData[7] = RxAVI_GetVic();
	ifData[8] =
	    0x00; // 0x00: limited range, 0x40: full range, only for YCbCr
	...
	for (i = 0; i < 14; i++) {
		...
		//		DEBUG_PRINT(MSG_ALWAYS, ("Configure Evita AVI
		//InfoFrame byte %2X to %2X\n", (int)i,
		//(int)(*pPacket[i+IF_HEADER_LENGTH-1])));
	}
	...
	if (0x19 >= 0x0C + i) {
		...
		DEBUG_PRINT(
		    MSG_ALWAYS,
		    ("Configure Evita AVI InfoFrame, force trigger here\n"));
	}

	// This bit will reset when overwrite infoframe
	if (YcMuxFlag) {
		SiiRegModify(PP_PAGE_3 | 0x60, BIT5,
			     SET_BITS); // Enable YC de MUX
	}
}
...
void SiiDrvEvitaAudioIfUpdate(uint8_t *pPacket)
{
	...
	for (i = 0; i < IF_MAX_AUDIO_LENGTH + 1 && i < data_length + 1; i++) {
		...
		//		DEBUG_PRINT(MSG_ALWAYS, ("Configure Evita AUDIO
		//InfoFrame byte %2X to %2X\n", (int)i,
		//(int)(*pPacket[i+IF_HEADER_LENGTH-1])));
	}
	...
	if (0xCD >= 0xC0 + i) {
		...
		DEBUG_PRINT(
		    MSG_ALWAYS,
		    ("Configure Evita ADUIO InfoFrame, force trigger here\n"));
	}
}
