...
static int lcd_tcon_deruma_setting_tl1(void)
{
	...
	data_cnt = (demura_setting[0] |
		    ...
		    (demura_setting[2] << 16) |
		    (demura_setting[3] << 24));
	if (data_cnt > tcon_rmem.demura_set_mem_size)
		data_cnt = tcon_rmem.demura_set_mem_size;
...
static int lcd_tcon_deruma_lut_tl1(void)
{
	...
	data_cnt = (demura_lut[0] |
		    ...
		    (demura_lut[2] << 16) |
		    (demura_lut[3] << 24));
	if (data_cnt > tcon_rmem.demura_lut_mem_size)
		data_cnt = tcon_rmem.demura_lut_mem_size;
...
static int lcd_tcon_vac_load(void)
{
	...
	if (data_cnt > tcon_rmem.vac_mem_size)
		data_cnt = tcon_rmem.vac_mem_size;
...
static int lcd_tcon_acc_lut_load(void)
{
	...
	if (data_cnt > tcon_rmem.acc_lut_mem_size)
		data_cnt = tcon_rmem.acc_lut_mem_size;
