...
#ifdef CONFIG_AMLOGIC_BL_HOTPLUG
static struct bl_hotplug_i2cdev *bl_hotplug;
#endif
...
#ifdef CONFIG_AMLOGIC_BL_HOTPLUG
void aml_bl_hotplug_set_m0516ldn(struct bl_hotplug_i2cdev *data)
{
	bl_hotplug = data;
	BLERR("bl_hotplug addr = %p\n", bl_hotplug);
}
EXPORT_SYMBOL(aml_bl_hotplug_set_m0516ldn);

void aml_bl_hotplug_set_a8552(struct bl_hotplug_i2cdev *data)
{
	bl_hotplug = data;
	BLERR("bl_hotplug addr = %p\n", bl_hotplug);
}
EXPORT_SYMBOL(aml_bl_hotplug_set_a8552);
#endif
...
struct aml_bl_drv_s *aml_bl_get_driver(void)
{
	...
	if (bl_drv == NULL)
		BLERR("aml_bl_get_driver:: no bl driver");
...
static void bl_power_on(void)
{
...
#endif
#ifdef CONFIG_AMLOGIC_BL_HOTPLUG
	case BL_CTRL_HOTPLUG:
		if (bl_hotplug == NULL) {
			BLERR("no bl_hotplug driver\n");
			goto exit_power_on_bl;
		}
		/* step 1: power on bl_hotplug */
		if (bl_hotplug->power_on) {
			ret = bl_hotplug->power_on(bl_hotplug);
			if (ret)
				BLERR("bl_hotplug: power on error\n");
		} else {
			BLERR("bl_hotplug: power on is null\n");
		}
	...
	bl_drv->state |= BL_STATE_BL_ON;
	bl_drv->bldev->props.power = BL_STATE_BL_ON;
...
static void bl_power_off(void)
{
	...
	case BL_CTRL_HOTPLUG:
		...
		/* step 1: power off bl_hotplug */
		if (bl_hotplug->power_off) {
			bl_hotplug->brightness = bl_drv->level;
			ret = bl_hotplug->power_off(bl_hotplug);
			if (ret)
				BLERR("bl_hotplug: power off error\n");
		} else {
			BLERR("bl_hotplug: power off is null\n");
		}
...
#endif
#ifdef CONFIG_AMLOGIC_BL_HOTPLUG
static void bl_set_level_hotplug(unsigned int level)
{
	int ret;
	...
	if (bl_hotplug == NULL) {
		...
		if (bl_hotplug->set_level) {
			bl_hotplug->brightness = level;
			ret = bl_hotplug->set_level(bl_hotplug, level);
			if (ret)
				BLERR("bl_hotplug: set_level error\n");
		} else {
			BLERR("bl_hotplug: set_level is null\n");
		}
	}
}
#endif
...
static int aml_bl_probe(struct platform_device *pdev)
{
	...
	props.power = 1;//FB_BLANK_UNBLANK; /* full on */
...
#ifdef CONFIG_AMLOGIC_LEGACY_EARLY_SUSPEND
		bl_early_suspend.level =
			EARLY_SUSPEND_LEVEL_DISABLE_FB;
		bl_early_suspend.suspend = bl_earlysuspend;
		bl_early_suspend.resume = bl_lateresume;
		bl_early_suspend.param = pdev;
