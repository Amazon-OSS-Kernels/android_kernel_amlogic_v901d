...
static int line_threshold = 20;
static int line_threshold_2 = 100;
...
void osd_set_window_axis_hw(u32 index, s32 x0, s32 y0, s32 x1, s32 y1)
{
	...
	osd_log_dbg2(MODULE_BLEND,
		     "osd_set_window_axis_hw: osd%d orin dst(x:%d y:%d w:%d h:%d)\n",
				   ...
				   int output_index)
{
	...
	if (osd_hw.osd_preblend_en && output_index == VIU1) {
		...
		osd_log_dbg2(MODULE_BLEND,
			    ...
			    blend_w, blend_h, x_offset, y_offset);
	...
	for (i = start_index; i < osd_count; i++) {
		...
		osd_log_dbg2(MODULE_BLEND, "pps_h_den:%d, pps_h_num=%d\n",
			pps_h_den, pps_h_num);
		osd_hw.dst_data[i].x =
			osd_hw.dst_data[i].x * pps_w_den / pps_w_num;
		osd_hw.dst_data[i].y =
			osd_hw.dst_data[i].y * pps_h_den / pps_h_num;
		osd_hw.dst_data[i].w =
			osd_hw.dst_data[i].w * pps_w_den / pps_w_num;
		osd_hw.dst_data[i].h =
			osd_hw.dst_data[i].h * pps_h_den / pps_h_num;
	...
	osd_log_dbg2(MODULE_BLEND, "orin disp(x:%d y:%d w:%d h:%d)\n",
	...
	osd_log_dbg2(MODULE_BLEND, "adjust disp(x:%d y:%d w:%d h:%d)\n",
		    osd_hw.disp_info[output_index].position_x,
		    osd_hw.disp_info[output_index].position_y,
		    osd_hw.disp_info[output_index].position_w,
		    osd_hw.disp_info[output_index].position_h);
...
static bool vd1_is_enable(void)
{
	u32 vd1_blend_src_trl;

	vd1_blend_src_trl   = osd_reg_read(VD1_BLEND_SRC_CTRL);
	if ((vd1_blend_src_trl & 0xf00) == (POSTBLD_VD1 << 8) ||
		((vd1_blend_src_trl & 0xf) == POSTBLD_VD1))
		return true;
	else
		return false;
}
...
static void set_blend_reg(struct layer_blend_reg_s *blend_reg)
{
	...
	if (osd_hw.osd_preblend_en) {
		if (vd1_is_enable())
			...
			blend_reg->postbld_src3_sel = POSTBLD_VD1;
	}
