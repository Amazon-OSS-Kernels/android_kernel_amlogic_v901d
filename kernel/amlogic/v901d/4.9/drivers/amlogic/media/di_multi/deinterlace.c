...
static int di_init_buf_simple(struct di_ch_s *pch)
{
	...
	/* local buf init */
	/**********************************************/
	tmp_meta = de_devp->local_meta_addr + ((de_devp->local_meta_size /
		DI_CHANNEL_NUB) * ch);
	...
	if (ppre->di_wr_buf->flg_nv21) {
		...
		if (cfgg(LINEAR)) {
			...
			ppre->di_nrwr_mif.addr =
				ppre->di_wr_buf->vframe->canvas0_config[0].
				phy_addr;
			ppre->di_nrwr_mif.addr1 =
				ppre->di_wr_buf->vframe->canvas0_config[1].
				phy_addr;
		}
		...
		//PR_INF("%s:%d\n", __func__, pch->rsc_bypass.b.ponly_fst_cnt);
			...
			if (ppre->prog_proc_type != 0x10 &&
				...
				if (dim_check_di_buf(di_buf_tmp, 10, channel)) {
					...
					channel);
					PR_ERR("DI:no free buffer P skip.\n");
					return 18;
				}
...
/* di_buf is local buf */
		...
		if ((dip_is_support_nv2110(channel)) && (ppre->sgn_lv == EDI_SGN_4K))
