	...
	)
{
	...
	unsigned int pic_hsize = mif->buf_crop_en ?
			mif->buf_hsize : mif->luma_x_end0 - mif->luma_x_start0
			+ 1;

// 0:8 bits 1:10 bits 422(old mode,12bit) 2: 10bit 444
...
//stimulus_display("di_mif0_stride: burst_stride_0    = %x\n",burst_stride_0);
	...
	*stride_y  = ((burst_stride_0 +  3) >> 2) << 2;//now need 64byte aligned
...
/* DI_MIF0_t -> DI_MIF_S*/
void di_mif0_linear_rd_cfg(struct DI_MIF_S *mif, int mif_index,
...
//	op->bwr(off + RDMIFXN_STRIDE_1, stride_cr, 0, 13);//stride
	...
	dbg_ic("\t:reg:0x%x= 0x%x\n", off + RDMIFXN_BADDR_Y, op->rd(off +
		RDMIFXN_BADDR_Y));
	dbg_ic("\t:reg:0x%x= 0x%x\n", off + RDMIFXN_BADDR_CB, op->rd(off +
		RDMIFXN_BADDR_CB));
	dbg_ic("\t:reg:0x%x= 0x%x\n", off + RDMIFXN_BADDR_CR, op->rd(off +
		RDMIFXN_BADDR_CR));
	dbg_ic("\t:reg:0x%x= 0x%x\n", off + RDMIFXN_STRIDE_0, op->rd(off +
		RDMIFXN_STRIDE_0));
	dbg_ic("\t:reg:0x%x= 0x%x\n", off + RDMIFXN_STRIDE_1, op->rd(off +
		RDMIFXN_STRIDE_1));
}

/* DI_MIF0_t -> DI_MIF_S*/
void di_mif0_linear_wr_cfg(struct DI_MIF_S *mif, int mif_index,
	const struct reg_acc *ops)
{
...
static void set_post_mtnrd_mif_t7(struct DI_SIM_MIF_s *mtnprd_mif)
{
	...
	di_mif1_linear_rd_cfg(mtnprd_mif, MTNRD_CTRL1,
		MTNRD_CTRL2, MTNRD_BADDR);
}
