...
unsigned int tm_debug = 1;
module_param(tm_debug, uint, 0664);
MODULE_PARM_DESC(tm_debug, "tm_debug\n");
	...
	struct vinfo_s *vinfo, enum vd_path_e vd_path)
{
	...
	if (video_process_flags[vd_path] & PROC_FLAG_FORCE_PROCESS) {
		pr_csc(4, "change_flag |= SIG_HDR10_PLUS_TM\n");
		change_flag |= SIG_HDR10_PLUS_TM;
	}
	...
	int force_source_lumin, enum vd_path_e vd_path)
{
	...
	pr_csc(4, "hdr10_plus_process_update.\n");
	hdr10_plus_ootf_gen(
		panel_lumin,
		force_source_lumin,
		&hdr10pgen_param);
	...
	enum vd_path_e vd_path)
{
	...
	if (!tm_debug) {
		pr_info("bypass vpp_matrix_update.\n");
		return 0;
	}
	...
	if ((vf != NULL) && ((flags & CSC_FLAG_TOGGLE_FRAME) ||
		(signal_change_flag & SIG_HDR10_PLUS_TM))) {
