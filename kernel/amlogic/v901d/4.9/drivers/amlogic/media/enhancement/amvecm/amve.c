...
void vpp_set_rgb_ogo(struct tcon_rgb_ogo_s *p)
{
	...
	if (video_rgb_ogo_xvy_mtx) {
		...
		if (get_cpu_type() >= MESON_CPU_MAJOR_ID_G12A) {
			VSYNC_WR_MPEG_REG_BITS(VPP_POST_MATRIX_EN_CTRL,
				p->en, 0, 1);
			VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_PRE_OFFSET0_1,
				((m[0] & 0xfff) << 16)
				| (m[1] & 0xfff));
			VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_PRE_OFFSET2,
				m[2] & 0xfff);
			VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_COEF00_01,
				((m[3] & 0x1fff) << 16)
				| (m[4] & 0x1fff));
			VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_COEF02_10,
				((m[5]	& 0x1fff) << 16)
				| (m[6] & 0x1fff));
			VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_COEF11_12,
				((m[7] & 0x1fff) << 16)
				| (m[8] & 0x1fff));
			VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_COEF20_21,
				((m[9] & 0x1fff) << 16)
				| (m[10] & 0x1fff));
			VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_COEF22,
				m[11] & 0x1fff);
			if (m[21]) {
				VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_COEF13_14,
					((m[12] & 0x1fff) << 16)
					| (m[13] & 0x1fff));
				VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_COEF15_25,
					((m[14] & 0x1fff) << 16)
					| (m[17] & 0x1fff));
				VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_COEF23_24,
					((m[15] & 0x1fff) << 16)
					| (m[16] & 0x1fff));
			}
			VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_OFFSET0_1,
				((m[18] & 0xfff) << 16)
				| (m[19] & 0xfff));
			VSYNC_WR_MPEG_REG(VPP_POST_MATRIX_OFFSET2,
				m[20] & 0xfff);
			VSYNC_WR_MPEG_REG_BITS(VPP_POST_MATRIX_CLIP,
				m[21], 3, 2);
			VSYNC_WR_MPEG_REG_BITS(VPP_POST_MATRIX_CLIP,
				m[22], 5, 3);
			return;
