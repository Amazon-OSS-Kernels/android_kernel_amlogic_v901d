...
static struct ppframe_s vfp_pool[VF_POOL_SIZE * 2];
...
static struct buf_status_s buf_status[VF_POOL_SIZE + 1];
...
((pp_vf >= &vfp_pool[0]) && (pp_vf <= &vfp_pool[(VF_POOL_SIZE * 2) - 1]))

void init_ppframe(void)
{
	memset(vfp_pool, 0, sizeof(vfp_pool));
}

int return_ppframe(struct vframe_s *vf)
{
	...
	if (vf) {
		pp_vf = to_ppframe(vf);
		if (pp_vf && is_valid_ppframe(pp_vf) && pp_vf->need_return) {
			pp_vf->need_return = false;
			return 0;
		}
	}
	return ret;
}
EXPORT_SYMBOL(return_ppframe);
...
static struct vframe_s *ppmgr_vf_peek(void *op_arg)
{
	...
	if (ppmgr_blocking || ppmgr_quit_flag)
		return NULL;
	vf = vfq_peek(&q_ready);
	return vf;
}
...
static struct vframe_s *ppmgr_vf_get(void *op_arg)
{
	...
	if (vf) {
		pp_vf = to_ppframe(vf);
		if ((vf->type & VIDTYPE_DI_PW) && pp_vf &&
			is_valid_ppframe(pp_vf))
			pp_vf->need_return = true;
...
static int get_source_type(struct vframe_s *vf)
{
	...
	if (ppmgr_device.force_composer)
		interlace_mode = 0;
	else
...
static int ppmgr_receiver_event_fun(int type, void *data, void *private_data)
{
	...
	case VFRAME_EVENT_PROVIDER_REG:
		ppmgr_device.sec_mem = ppmgr_check_tvp_state();
		PPMGRVPP_INFO("register now, old tvp flag: %d, new flag: %d\n",
			ppmgr_device.cur_sec_mem, ppmgr_device.sec_mem);
		break;
...
void vf_local_init(void)
{
	...
	j = 0;
	for (i = 0; i < VF_POOL_SIZE * 2; i++) {
		if (vfp_pool[i].need_return)
			continue;
		vfp_pool[i].index = j;
	...
	if (j < VF_POOL_SIZE)
		PPMGRVPP_INFO("ppframe buffer is leak,free cnt:%d, total:%d\n",
			j, VF_POOL_SIZE * 2);
}
...
static int local_canvas_init(void)
{
	...
	for (i = 0; i < 2; i++)
		ppmgr_dst_canvas[i] = PPMGR_CANVAS_INDEX + i + 3;
...
int ppmgr_vf_put_dec(struct vframe_s *vf)
{
#if 0
		...
		int angle)
{
	...
	int max_value;
	u32 ar = (vf->ratio_control >> DISP_RATIO_ASPECT_RATIO_BIT) & 0x3ff;
	...
	if (ppmgr_device.ppmgr_debug & 1) {
		PPMGRVPP_INFO("disp_width: %d, disp_height: %d\n",
			disp_w, disp_h);
		PPMGRVPP_INFO("input_width: %d, input_height: %d, in_ar: %d\n",
			input_width, input_height, ar);
	}
	if (ar == 0x3ff)
		ar = div_u64((u64)256ULL * (u64)vf->sar_height *
			     (u64)input_height,
			     (u32)(vf->sar_width * input_width));
	...
	if (angle & 1) {
		...
		if (input_width < disp_w &&
			...
			if ((input_width * disp_h) > (disp_w * input_height)) {
				...
				if (ar == 0 || ar == 0x3ff)
					h =
					disp_w * input_height / input_width;
				else
					h = (ar * w) >> 8;
				if (h > disp_h) {
					w = (w * disp_h) / h;
				...
				if (ar == 0)
					w =
					disp_h * input_width / input_height;
				else
					w = (h << 8) / ar;
				if (w > disp_w) {
					h = (h * disp_w) / w;
	...
	if ((angle & 1) && (input_width > input_height))
		max_value = MAX_WIDTH;
	else
		...
		struct config_para_ex_s *ge2d_config)
{
	...
	} else if (!(ppmgr_device.ppmgr_debug & 8)) {
	...
	ret = ppmgr_buffer_init(ppmgr_device.cur_sec_mem);
	...
	new_vf->flag = vf->flag;
	new_vf->mem_handle = buf_status[pp_vf->index].mem_handle;
	buf_start = buf_status[pp_vf->index].mem_handle->phy_addr;
	...
	if (ppmgr_device.force_composer) {
		new_vf->canvas0_config[0].width = ppmgr_device.disp_width * 3;
		new_vf->canvas0_config[0].height = ppmgr_device.disp_height;
	} else {
	...
	if (buf_status[pp_vf->index].dirty == 1) {
		...
		ge2d_config->mem_sec = ppmgr_device.cur_sec_mem;
		...
		fillrect(context, 0, 0, ppmgr_device.disp_width,
			 ppmgr_device.disp_height, 0x008080ff);
		memset(ge2d_config, 0, sizeof(struct config_para_ex_s));
	}
	...
	if (strstr(ppmgr_device.dump_path, "scr")
		...
		if (IS_ERR(filp_scr))
			...
			PPMGRVPP_INFO("scr addr: %u, width: %u, height: %u\n",
				vf->canvas0_config[0].phy_addr,
				vf->canvas0_config[0].width,
				vf->canvas0_config[0].height);
		...
		struct config_para_ex_s *ge2d_config)
{
	...
	buf_start = buf_status[VF_POOL_SIZE].mem_handle->phy_addr;
	...
	vf->canvas1Addr = -1;
	vf->plane_num = 1;
	vf->mem_handle = buf_status[pp_vf->index].mem_handle;
	buf_start = buf_status[pp_vf->index].mem_handle->phy_addr;
	dst_canvas_id = ppmgr_dst_canvas[0];
...
static int ppmgr_task(void *data)
{
	...
	ppmgr_device.cur_sec_mem = (ppmgr_device.sec_mem == 1) ? 1 : 0;
...
SKIP_DETECT:
		...
		if (ppmgr_blocking) {
			...
			PPMGRVPP_INFO("ppmgr rebuild light-unregister_1\n");
			vf_unreg_provider(&ppmgr_vf_prov);
			...
			up(&ppmgr_device.ppmgr_sem);
			PPMGRVPP_INFO("ppmgr rebuild light-unregister_2\n");
			PPMGRVPP_INFO(
				"ppmgr,reset,free %d, avail %d\n",
				vfq_level(&q_free),
				vfq_level(&q_ready));
		}
		if (ppmgr_device.ppmgr_debug & 1)
			PPMGRVPP_WARN(
				"ppmgr, dec %p,free %d, avail %d\n",
				ppmgr_vf_peek_dec(),
				vfq_level(&q_free),
				vfq_level(&q_ready));
	...
	destroy_ge2d_work_queue(context);
	{
		...
		while (!vfq_empty(&q_ready)) {
			vf_local = vfq_pop(&q_ready);
			pp_vf = to_ppframe(vf_local);
			if (pp_vf && pp_vf->dec_frame &&
			    (pp_vf->dec_frame->type & VIDTYPE_DI_PW)) {
				dim_post_keep_cmd_release2(pp_vf->dec_frame);
				i++;
			}
		}
		if (i > 0)
			PPMGRVPP_INFO("ppmgr recycle di buffer %d\n", i);
	}
	local_canvas_uninit();
...
int ppmgr_buffer_uninit(void)
{
	...
	if (!ppmgr_device.use_reserved &&
	    ppmgr_device.alloc_done) {
		PPMGRVPP_INFO("cma free size is %x\n",
			ppmgr_device.buffer_size);
		for (i = 0; i < VF_POOL_SIZE + 1; i++) {
			if (buf_status[i].mem_handle)
				codec_mm_release(buf_status[i].mem_handle,
					"ppmgr");
			buf_status[i].mem_handle = NULL;
		}
		ppmgr_device.alloc_done = false;
		ppmgr_device.buffer_start = 0;
		ppmgr_device.buffer_size = 0;
		PPMGRVPP_INFO("cma free done\n");
	}
...
int ppmgr_buffer_init(int secure_mode)
{
	...
	if (!ppmgr_device.use_reserved &&
		...
		for (i = 0; i < VF_POOL_SIZE + 1; i++) {
			mm = codec_mm_alloc("ppmgr", decbuf_size, 0, flags);
			if (mm) {
				buf_status[i].mem_handle = mm;
				ppmgr_device.buffer_size += decbuf_size;
				ppmgr_device.alloc_done = true;
			} else
				break;
		}
		PPMGRVPP_INFO("cma memory size is %x\n",
			ppmgr_device.buffer_size);
		if (ppmgr_device.buffer_size <
		    decbuf_size * (VF_POOL_SIZE + 1)) {
			ppmgr_buffer_status = 2;
...
static int tb_buffer_init(void)
{
	...
	if (tb_buffer_start == 0) {
		...
		for (i = 0; i < tb_buffer_len; i++) {
			...
			if (ppmgr_device.tb_detect & 0xc) {
				PPMGRVPP_INFO(
					"detect buff(%lu) paddr: %lu, vaddr: %lu\n",
					i,
					detect_buf[i].paddr,
					detect_buf[i].vaddr);
			}
