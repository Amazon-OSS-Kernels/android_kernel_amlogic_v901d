 ...
 */
	...
	if (devp->parm.reserved & PARAM_STATE_WR_MEM_ORDER_STD) {
		...
		wr_bits(offset, VDIN_WR_CTRL, 0, WORDS_SWAP_BIT,
		WORDS_SWAP_WID);
		/* little endian */
		wr_bits(offset, VDIN_WR_H_START_END, 1, WR_ENDIAN_BIT,
		WR_ENDIAN_WID);
	} else {
		/* swap words */
		wr_bits(offset, VDIN_WR_CTRL, 1, WORDS_SWAP_BIT,
		WORDS_SWAP_WID);
		/* big endian */
		wr_bits(offset, VDIN_WR_H_START_END, 0, WR_ENDIAN_BIT,
		WR_ENDIAN_WID);
	}
}
			...
			unsigned int canvas_id)
{
	...
	if (rdma_enable) {
		...
		if (devp->pause_dec)
			rdma_write_reg_bits(devp->rdma_handle, VDIN_WR_CTRL +
				devp->addr_offset, 0, WR_REQ_EN_BIT,
				WR_REQ_EN_WID);
		else
			rdma_write_reg_bits(devp->rdma_handle, VDIN_WR_CTRL +
				devp->addr_offset, 1, WR_REQ_EN_BIT,
				WR_REQ_EN_WID);
	} else
#endif
		{
