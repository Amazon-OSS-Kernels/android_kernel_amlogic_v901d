...
bool neo_dbg_en;
module_param(neo_dbg_en, bool, 0664);
MODULE_PARM_DESC(neo_dbg_en, "enable/disable vdin debug information");
...
unsigned int cinemo_cut_vs;
module_param(cinemo_cut_vs, uint, 0664);
MODULE_PARM_DESC(cinemo_cut_vs, "cinemo_cut_vs");

unsigned int cinemo_cut_ve;
module_param(cinemo_cut_ve, uint, 0664);
MODULE_PARM_DESC(cinemo_cut_ve, "cinemo_cut_ve");
...
#ifdef CONFIG_IDME
	if (devp->parm.port == TVIN_PORT_VIU1_WB0_VPP) {
		devp->prop.vs = cinemo_cut_vs;
		devp->prop.ve = cinemo_cut_ve;
	}
#endif
...
int start_tvin_service(int no, struct vdin_parm_s  *para)
{
	...
	if (para->frame_rate == 0) {
		pr_err("[vdin]%s vdin%d frame_rate is zero %d\n",
		       __func__, no, para->frame_rate);
		return -1;
	}
...
static long vdin_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TVIN_IOC_S_VDIN_V4L2START:
		...
		if ((!strncmp(model_name, "FHD", 3)) ||
				(!strncmp(model_name, "APTIV", 5))) {
			if (param.port == TVIN_PORT_VIU1_WB0_VPP) {
				if (param.h_active == 1280) {
					param.v_active = 800;
					cinemo_cut_vs = 40;
					cinemo_cut_ve = 40;
				} else if (param.h_active == 1920) {
					param.v_active = 1200;
					cinemo_cut_vs = 60;
					cinemo_cut_ve = 60;
				}
	...
	case TVIN_IOC_G_BUF:
		...
		if (!vfe) {
			if (vdin_dbg_en)
				pr_info("TVIN_IOC_G_BUF: can not peek buf,irq cnt:%d\n",
					devp->irq_cnt);
			ret = -EFAULT;
			break;
		}
		...
		if (!vfe) {
			if (vdin_dbg_en)
				pr_info("TVIN_IOC_G_BUF:can not get buf, irq cnt:%d\n",
					devp->irq_cnt);
			ret = -EFAULT;
			break;
		}
...
static int vdin_drv_probe(struct platform_device *pdev)
{
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1) && (vdevp->index == 0)) {
		...
		if (vdevp->afbce_flag & 0x1) {
			vdevp->afbce_info =
				devm_kzalloc(vdevp->dev, sizeof(struct
				vdin_afbce_s), GFP_KERNEL);
			if (!vdevp->afbce_info)
				goto fail_kzalloc_vdev;
		}
	}
