...
unsigned int vdin_cma_alloc(struct vdin_dev_s *devp)
{
	 ...
	 * dolby has one page size, & each vframe aligned to
	 * page size
 ...
 */
	...
	if (devp->cma_config_flag & MEM_ALLOC_DISCRETE) {
				...
				for (j = 0; j < 20; j++) {
					devp->vfmem_start[i] =
						codec_mm_alloc_for_dma(
							vdin_name, frame_size /
							PAGE_SIZE, 0, flags);
					if (devp->vfmem_start[i] == 0) {
						msleep(50);
						pr_err(
						"alloc mem fail:50*%dms\n", j);
					} else {
						break;
					}
				}
		...
		if (devp->cma_config_flag & MEM_ALLOC_FROM_CODEC) {
			devp->mem_start =
				codec_mm_alloc_for_dma(vdin_name, mem_size /
				PAGE_SIZE, 0, flags);
	...
	if (devp->cma_config_flag & MEM_ALLOC_DISCRETE) {
		...
		if (devp->cma_config_flag & MEM_ALLOC_FROM_CODEC) {
			...
			if (devp->venc_pages && devp->mem_size)
				dma_release_from_contiguous(
				&devp->this_pdev->dev, devp->venc_pages,
