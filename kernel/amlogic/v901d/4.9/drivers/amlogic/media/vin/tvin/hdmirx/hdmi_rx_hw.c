...
bool is_clk_stable(void)
{
	...
	if (rx.chip_id >= CHIP_ID_TL1) {
		if ((rx.phy_ver == PHY_VER_TM2) && force_sqo)
			return true;
		/* sqofclk */
...
static int TOP_init(void)
{
	int err = 0;
	int data32 = 0;
	int board_id = DV_DEVICE; /* default as DV device */
	int project_id = LAGUNAF; /* default as DV device */
	...
	if ((!strncmp(model_name, "HD", LEN_HD)) ||
		(!strncmp(model_name, "1080P", LEN_1080P)))
		project_id = EVOLVE;

	if (!strncmp(model_name, "APTIV", 5))
		rx.RUmodel = true;
	else
		rx.RUmodel = false;
#endif
	if ((board_id >= DV_DEVICE) || (project_id == EVOLVE))
		if (rx.chip_id == CHIP_ID_GXTVBB)
			data32 |= 0 << 5;
		else
			data32 |= 1 << 5;
	else
		if ((rx.chip_id == CHIP_ID_GXTVBB) || (rx.chip_id == CHIP_ID_TM2))
			data32 |= 0 << 5;
		else
			data32 |= 1 << 5;
...
/* TL1:TODO...*/
bool is_low_amplitude_sig(void)
{
	u32 data32;
	...
	rx_pr("sqo check:ch0=%x,ch1=%x,ch2=%x,avr=%x\n", ch0_tap0, ch1_tap0,
		ch2_tap0, avr_tap0);
	if (avr_tap0 > 10) {
		ret = true;
		if (log_level & EQ_LOG)
	...
	if (rx.phy.phy_bw >= PHY_BW_4) {
		...
		while (vga_dbg) {
			...
			if ((vga_val < 30) && (vga_val > 0))
				wr_reg_hhi_bits(HHI_HDMIRX_PHY_DCHA_CNTL0,
					MSK(15, 0), ((data32 << 1) | 0x1));
			/* 3g dont need turn off DFE */
			//else
			/* turn off dfe after vga done */
			//wr_reg_hhi_bits(
			//HHI_HDMIRX_PHY_DCHD_CNTL2,
			//DFE_EN, 0);
			/*vga_val will not biger than 60*/
			/*comment it out*/
			//else if (vga_val > 60)
			//wr_reg_hhi_bits(
			//HHI_HDMIRX_PHY_DCHA_CNTL0,
		...
		/* low_amplitude */
		force_sqo = is_low_amplitude_sig();
		if (force_sqo)
