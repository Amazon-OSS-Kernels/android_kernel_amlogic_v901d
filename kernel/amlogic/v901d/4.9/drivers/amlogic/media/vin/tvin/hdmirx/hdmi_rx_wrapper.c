...
static int pow5v_max_cnt_LONTIUM = 400;
static int judge_pow5v = 3;
...
int sil5293_source_det;
EXPORT_SYMBOL(sil5293_source_det);
static bool g_last_sil5293_flag;
...
int rx_set_global_variable(const char *buf, int size)
{
	...
	if (set_pr_var(tmpbuf, pow5v_max_cnt_LONTIUM, value, &index, ret))
		return pr_var(pow5v_max_cnt_LONTIUM, index);
	if (set_pr_var(tmpbuf, judge_pow5v, value, &index, ret))
		return pr_var(judge_pow5v, index);
	...
	if (set_pr_var(tmpbuf, force_sqo, value, &index, ret))
		return pr_var(force_sqo, index);
...
void rx_get_global_variable(const char *buf)
{
	...
	pr_var(pow5v_max_cnt_LONTIUM, i++);
	pr_var(judge_pow5v, i++);
...
void hdmirx_open_port(enum tvin_port_e port)
{
	...
	if (rx.port == 2) {
		judge_pow5v = pow5v_max_cnt_LONTIUM;
	} else {
		judge_pow5v = pow5v_max_cnt;
	}
...
void rx_5v_monitor(void)
{
	...
	if (sil5293_source_det == 1 && g_last_sil5293_flag == false)
		g_last_sil5293_flag = true;
	if (sil5293_source_det == 0 && g_last_sil5293_flag == true) {
		g_last_sil5293_flag = false;
		if ((rx.RUmodel == true) && (rx.port == 1)) {
			rx_pr("sil5293 debug source removed!!\n");
	...
	if (check_cnt >= judge_pow5v) {
		check_cnt = 0;
		pwr_sts = tmp_5v;
		rx_update_fastswitch_sts(pwr_sts);
		if ((rx.RUmodel == true) && (rx.port == 1))
			rx.cur_5v_sts = g_last_sil5293_flag ? 1 : 0;
		else
			rx.cur_5v_sts = (pwr_sts >> rx.port) & 1;
		hotplug_wait_query();
		rx_pr("hotplug-0x%x\n", pwr_sts);
		if (rx.cur_5v_sts == 0) {
			rx_pr("sil5293 debug ========== source removed V2!!\n");
			set_fsm_state(FSM_5V_LOST);
			rx.err_code = ERR_5V_LOST;
			vic_check_en = false;
			/* dvi_check_en = true; */
