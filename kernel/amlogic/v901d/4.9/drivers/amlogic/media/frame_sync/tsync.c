...
static ssize_t show_video_only_mode(struct class *class,
	struct class_attribute *attrr, char *buf)
{
	return sprintf(buf, "%d\n", video_only_mode);
}

static ssize_t store_video_only_mode(struct class *class,
	 struct class_attribute *attr, const char *buf, size_t size)
{
	unsigned int mode;
	ssize_t r;
	...
	pr_info("[%s]video_only_mode=%d, buf=%s\n",
		__func__, video_only_mode, buf);
	return size;
}
...
static struct class_attribute tsync_class_attrs[] = {
	...
	store_apts_lookup),
	__ATTR(video_only, 0644, show_video_only_mode,
	store_video_only_mode),
...
int tsync_store_fun(const char *trigger, int id, const char *buf, int size)
{
	...
	case 21:return -1;
	case 22:return -1;
	case 23:return store_need_videodrop(NULL, NULL, buf, size);
	default:
		ret = -1;
	}
	return size;
...
static struct mconfig tsync_configs[] = {
	...
	MC_FUN_ID("need_drop_video", tsync_show_fun, tsync_store_fun, 23),
};
