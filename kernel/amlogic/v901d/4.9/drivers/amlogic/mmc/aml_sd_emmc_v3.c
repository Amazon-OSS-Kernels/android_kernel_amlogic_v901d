		...
		u32 adj, u32 div, u32 core_phase)
{
	...
	tmp[0] = map;
	left = find_last_bit(tmp, div);
	right = find_first_bit(tmp, div);
	mid = find_first_zero_bit(tmp, div);
	size = left - right + 1;
		...
		u32 shift, u32 div)
{
	...
	tmp[0] = map;
	bitmap_shift_right(right, tmp,
			   shift, div);
	bitmap_shift_left(left, tmp,
			  div - shift, div);
	bitmap_or(tmp, right, left, div);
		...
		struct amlsd_platform *pdata, bool no_cmd)
{
	...
	if (aml_card_type_mmc(pdata)) {
		...
		if (no_cmd) {
			writel(AML_MV_DLY2_NOCMD(step), host->base +
			SD_EMMC_DELAY2_V3);
			pdata->dly2 = AML_MV_DLY2_NOCMD(step);
		} else {
			writel(AML_MV_DLY2(step), host->base +
			SD_EMMC_DELAY2_V3);
		...
		if (!no_cmd) {
			writel(AML_MV_DLY2_NOMMC_CMD(step), host->base +
			SD_EMMC_DELAY2_V3);
