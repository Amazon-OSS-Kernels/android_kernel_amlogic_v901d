/*
 * drivers/amlogic/uart/voxx_vip_uart.c
 *
...
#if defined(CONFIG_VOXX_VIP_UART) && defined(CONFIG_MAGIC_SYSRQ)
...
static struct uart_driver voxx_vip_uart_driver;
unsigned int vip_xtal_tick_en;
struct voxx_vip_uart_port {
...
#define to_voxx_vip_port(uport) \
	container_of(uport, struct voxx_vip_uart_port, port)
static struct voxx_vip_uart_port *voxx_vip_ports[AML_UART_PORT_MAX];

struct uart_port *get_voxx_vip_uart_port(int id)
{
	struct uart_port *port;

	port = &voxx_vip_ports[id]->port;
	return port;
}
EXPORT_SYMBOL(get_voxx_vip_uart_port);
static int voxx_vip_uart_setup(struct console *co, char *options);

#define VOXX_VIP_UART_BUFFER_SIZE (1024 * 128)
...
struct voxx_vip_uart_management {
	int user_count;
	struct list_head list_head;
	spinlock_t lock;
};
struct voxx_vip_uart_list {
	int user_count;
	struct voxx_vip_uart_struct *co_head;
	struct voxx_vip_uart_struct *co_tail;
};

static struct voxx_vip_uart_management cur_col_management;
static struct voxx_vip_uart_list cur_col_list[AML_UART_PORT_MAX];
static char *data_cache;
static int voxx_vip_uart_index =  -1;
...
static void get_next_node(struct voxx_vip_uart_list *cur_col,
	struct voxx_vip_uart_struct *co_struct, int index)
{
	...
	if (!list_empty(list_head)) {
		...
		list_for_each_prev(entry, list_head) {
			co_struct = list_entry(entry,
				struct voxx_vip_uart_struct, list);
			co = co_struct->co;
			if (co->index == index)
				break;
		}
...
static int print_more(struct voxx_vip_uart_port *mup)
{
	int index = 0;
	struct voxx_vip_uart_struct *co_struct = NULL;
	struct voxx_vip_uart_port *mup_tmp = NULL;
	struct voxx_vip_uart_list *cur_col = NULL;
	struct console *co = NULL;
	const char *s = NULL;
	...
	if (index != voxx_vip_uart_index)
		voxx_vip_uart_setup(co, NULL);
...
static void voxx_vip_uart_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
}

static unsigned int voxx_vip_uart_get_mctrl(struct uart_port *port)
{
	return TIOCM_CTS;
}

static unsigned int voxx_vip_uart_tx_empty(struct uart_port *port)
{
	u32 val;
...
static void voxx_vip_uart_stop_tx(struct uart_port *port)
{
	u32 val;
...
static void voxx_vip_uart_stop_rx(struct uart_port *port)
{
	u32 val;
...
static void voxx_vip_uart_shutdown(struct uart_port *port)
{
	unsigned long flags;
	u32 val;
...
static void voxx_vip_uart_start_tx(struct uart_port *port)
{
	struct circ_buf *xmit = &port->state->xmit;
	unsigned int ch;
	struct voxx_vip_uart_port *mup = to_voxx_vip_port(port);
	unsigned long flags;
...
static void voxx_vip_transmit_chars(struct uart_port *port)
{
	struct circ_buf *xmit = &port->state->xmit;
	struct voxx_vip_uart_port *mup = to_voxx_vip_port(port);
	unsigned int ch;
...
static void voxx_vip_receive_chars(struct uart_port *port)
{
...
static irqreturn_t voxx_vip_uart_interrupt(int irq, void *dev_id)
{
...
static const char *voxx_vip_uart_type(struct uart_port *port)
{
	/* TO DO -xl */
	return (port->type == PORT_MESON) ? "voxx_vip_uart" : NULL;
}

static int voxx_vip_uart_startup(struct uart_port *port)
{
	u32 val;
	int ret = 0;
...
static void voxx_vip_uart_change_speed(struct uart_port *port,
			unsigned long baud)
{
	u32 val;
	struct voxx_vip_uart_port *mup = to_voxx_vip_port(port);
...
static void voxx_vip_uart_set_termios(struct uart_port *port,
				   ...
				   struct ktermios *old)
{
	...
	voxx_vip_uart_change_speed(port, baud);
...
static int voxx_vip_uart_verify_port(struct uart_port *port,
...
static void voxx_vip_uart_release_port(struct uart_port *port)
{
...
static int voxx_vip_uart_request_port(struct uart_port *port)
{
	struct platform_device *pdev = to_platform_device(port->dev);
	struct resource *res;
	int size, ret;
	...
	ret = request_irq(port->irq, voxx_vip_uart_interrupt, 0,
			  voxx_vip_uart_type(port), port);
	return 0;
}

static void voxx_vip_uart_config_port(struct uart_port *port, int flags)
{
	if (flags & UART_CONFIG_TYPE) {
		port->type = PORT_MESON;
		voxx_vip_uart_request_port(port);
	}
}

static struct uart_ops const voxx_vip_uart_ops = {
	.set_mctrl = voxx_vip_uart_set_mctrl,
	.get_mctrl = voxx_vip_uart_get_mctrl,
	.tx_empty = voxx_vip_uart_tx_empty,
	.start_tx = voxx_vip_uart_start_tx,
	.stop_tx = voxx_vip_uart_stop_tx,
	.stop_rx = voxx_vip_uart_stop_rx,
	.startup = voxx_vip_uart_startup,
	.shutdown = voxx_vip_uart_shutdown,
	.set_termios = voxx_vip_uart_set_termios,
	.type = voxx_vip_uart_type,
	.config_port = voxx_vip_uart_config_port,
	.request_port = voxx_vip_uart_request_port,
	.release_port = voxx_vip_uart_release_port,
	.verify_port = voxx_vip_uart_verify_port,
};

#ifdef CONFIG_VOXX_VIP_UART

static void voxx_vip_console_putchar(struct uart_port *port, int ch)
{
	if (!port->membase)
		return;
...
static void voxx_vip_uart_write(struct console *co, const char *s,
				       u_int count)
{
	struct uart_port *port;

	const char *cur_s = NULL;
	struct voxx_vip_uart_struct *head_s = NULL;
	struct voxx_vip_uart_struct *tail_s = NULL;
	struct list_head *list_head = NULL;
	struct voxx_vip_uart_struct *co_struct = NULL;
	struct voxx_vip_uart_list *cur_col = NULL;
	long tmp_count = 0;
	long tmp_count_1 = 0;
	long struct_size = sizeof(struct voxx_vip_uart_struct);
	int need_default = 0;
	static int last_msg_full;
	unsigned long flags = 0;
	int index = co->index;

	port = &voxx_vip_ports[co->index]->port;
	if (!port)
		return;
	if (!new_printk_enabled || !data_cache) {
		uart_console_write(port, s, count, voxx_vip_console_putchar);
		return;
	}

	spin_lock_irqsave(&cur_col_management.lock, flags);

	if (count > 0 && count < VOXX_VIP_UART_BUFFER_SIZE) {
		list_head = &cur_col_management.list_head;
		cur_s = NULL;
		if (list_empty(list_head)) {
			cur_s = data_cache;
		...
		if (need_default == 0) {
			co_struct = (struct voxx_vip_uart_struct *)cur_s;
			...
			if (last_msg_full == 0) {
				co_struct =
				(struct voxx_vip_uart_struct *)cur_s;
				co_struct->s = cur_s + struct_size;
...
static int voxx_vip_uart_setup(struct console *co, char *options)
{
	...
	voxx_vip_uart_index = (int)co->index;
...
static void voxx_vip_early_write(struct console *con, const char *s,
	unsigned int n)
{
	struct earlycon_device *dev = con->data;

	uart_console_write(&dev->port, s, n, voxx_vip_console_putchar);
}

static int __init voxx_vip_early_uart_setup(struct earlycon_device *device,
					    const char *opt)
{
	if (!device->port.membase)
		return -ENODEV;

	device->con->write = voxx_vip_early_write;
	return 0;
}
EARLYCON_DECLARE(aml_uart, voxx_vip_early_uart_setup);
EARLYCON_DECLARE_COMP(aml-uart, voxx_vip_early_uart_setup);

static struct console voxx_vip_uart = {
	.name = AML_UART_DEV_NAME,
	.write = voxx_vip_uart_write,
	.device = uart_console_device,
	.setup = voxx_vip_uart_setup,
	.flags = CON_PRINTBUFFER,
	.index = -1,
	.data = &voxx_vip_uart_driver,
};

static int __init voxx_vip_serial_uart_init(void)
{
	register_console(&voxx_vip_uart);
	return 0;
}

console_initcall(voxx_vip_serial_uart_init);

#define VOXX_VIP_UART	(&voxx_vip_uart)
#else
#define VOXX_VIP_UART	NULL
#endif

static struct uart_driver voxx_vip_uart_driver = {
	.owner = THIS_MODULE,
	.driver_name = "voxx_vip_uart",
	.dev_name = AML_UART_DEV_NAME,
	.nr = AML_UART_PORT_MAX,
	.cons = NULL,
};

static int voxx_vip_uart_probe(struct platform_device *pdev)
{
	struct resource *res_mem, *res_irq;
	struct uart_port *port;
	struct voxx_vip_uart_port *mup;
	struct clk *clk;
	const void *prop;
	int ret = 0;
	...
	if (voxx_vip_ports[pdev->id]) {
		pr_err("port %d already allocated\n", pdev->id);
		return -EBUSY;
	}

	mup = devm_kzalloc(&pdev->dev,
		sizeof(struct voxx_vip_uart_port), GFP_KERNEL);
	if (!mup)
		return -ENOMEM;
	...
	if (!vip_xtal_tick_en) {
		...
		"vip_xtal_tick_en", NULL);
		if (prop) {
			vip_xtal_tick_en = of_read_ulong(prop, 1);
			if (vip_xtal_tick_en == 1)
				vip_xtal_tick_en = 0;
		}
	}
	...
	port->type = PORT_MESON;
	port->x_char = 0;
	port->ops = &voxx_vip_uart_ops;

	voxx_vip_ports[pdev->id] = mup;
	...
	ret = uart_add_one_port(&voxx_vip_uart_driver, port);
	if (ret)
		voxx_vip_ports[pdev->id] = NULL;
...
static int voxx_vip_uart_remove(struct platform_device *pdev)
{
	struct uart_port *port;

	port = platform_get_drvdata(pdev);
	uart_remove_one_port(&voxx_vip_uart_driver, port);
	voxx_vip_ports[pdev->id] = NULL;

	return 0;
}

static int voxx_vip_uart_resume(struct platform_device *pdev)
{
	struct uart_port *port;
	u32 val;
	...
	if (port->line == 0)
		return 0;
	uart_resume_port(&voxx_vip_uart_driver, port);
...
static int voxx_vip_uart_suspend(struct platform_device *pdev,
	pm_message_t state)
{
	struct uart_port *port;
	u32 val;
	...
	if (port->line == 0)
		return 0;
	uart_suspend_port(&voxx_vip_uart_driver, port);
...
static const struct of_device_id voxx_vip_uart_dt_match[] = {
	{.compatible = "lab126, voxx_vip-uart"},
	{ /* sentinel */ },
};

MODULE_DEVICE_TABLE(of, voxx_vip_uart_dt_match);

static struct platform_driver voxx_vip_uart_platform_driver = {
	.probe = voxx_vip_uart_probe,
	.remove = voxx_vip_uart_remove,
	.suspend	= voxx_vip_uart_suspend,
	.resume		= voxx_vip_uart_resume,
	.driver = {
		   .owner = THIS_MODULE,
		   .name = "voxx_vip_uart",
		   .of_match_table = voxx_vip_uart_dt_match,
		   },
};

static int __init voxx_vip_uart_init(void)
{
	int ret;

	ret = uart_register_driver(&voxx_vip_uart_driver);
	if (ret)
		return ret;

	ret = platform_driver_register(&voxx_vip_uart_platform_driver);
	if (ret) {
		uart_unregister_driver(&voxx_vip_uart_driver);
		return ret;
	}

	ret = driver_create_file(&voxx_vip_uart_platform_driver.driver,
	...
	data_cache = vmalloc(VOXX_VIP_UART_BUFFER_SIZE);
	if (!data_cache) {
		pr_info("buffer alloc failed for uart\n");
		platform_driver_unregister(&voxx_vip_uart_platform_driver);
		uart_unregister_driver(&voxx_vip_uart_driver);
		return -ENOMEM;
	}
...
static void __exit voxx_vip_uart_exit(void)
{
	platform_driver_unregister(&voxx_vip_uart_platform_driver);
	uart_unregister_driver(&voxx_vip_uart_driver);
}

module_init(voxx_vip_uart_init);
module_exit(voxx_vip_uart_exit);

MODULE_AUTHOR("Carlo Caione <carlo@caione.org>");
MODULE_DESCRIPTION("Voxx VIP uart driver");
