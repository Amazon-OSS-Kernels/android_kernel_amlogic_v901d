...
int (*m0516ldn_audio_set_mute_ptr)(u8 value) = NULL;
EXPORT_SYMBOL(m0516ldn_audio_set_mute_ptr);
...
static int life_cycle_reason_lookup(void)
{
	...
	pr_info("soc_state_pwm [%s] smode is 0x%x\n", __func__, (int)s_mode);
	if (s_mode == LIFE_CYCLE_SMODE_FACTORY_RESET ||
		s_mode == LIFE_CYCLE_SMODE_POST_RECOVERY) {
...
int sys_m0516ldn_audio_set_mute(u8 value)
{
	int ret = -1;

	if (m0516ldn_audio_set_mute_ptr != NULL)
		ret = m0516ldn_audio_set_mute_ptr(value);
	else
		pr_err("no m0516ldn driver loaded for audio mute\n");
	return ret;
}

void mute_audio(u8 value)
{
	int ret = -1;
	ret = sys_m0516ldn_audio_set_mute(value);
	if (ret != -1)
		pr_info("mute m0516ldn audio\n");
}
EXPORT_SYMBOL(mute_audio);
...
int life_cycle_set_boot_reason(life_cycle_reason_t boot_reason)
{
	...
	mute_audio(0x00);
	pr_info("soc_state_pwm [%s] boot reason = 0x%x\n", __func__, (int) boot_reason);
	return 0;
}
...
int life_cycle_set_special_mode(life_cycle_reason_t life_cycle_special_mode)
{
	...
	mute_audio(0x00);
	pr_info("soc_state_pwm [%s] special boot reason = 0x%x\n", __func__, (int) life_cycle_special_mode);
	return 0;
}
EXPORT_SYMBOL(life_cycle_set_special_mode);

/*
 * life_cycle_get_mode_index
 * Description: this function will get the mode index in the lcr_data table
 * which causing device life cycle change
 * @Return the index of the reboot reason in lcr_data table
 */
int life_cycle_get_mode_index(void)
{
	return p_dev_sol->life_cycle_reason_idx;
}
EXPORT_SYMBOL(life_cycle_get_mode_index);
...
static int __init dev_sol_init(void)
{
	...
	pr_info("soc_state_pwm [%s] life cycle reason is %s\n", __func__,
		lcr_data[p_dev_sol->life_cycle_reason_idx].life_cycle_reasons);
