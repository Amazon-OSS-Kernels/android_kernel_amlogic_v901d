...
/*
 * FHD panel true touch grid is 1920x1200 for a 1920x1200 panel.But display
 * surface is only 1920x1080 from apps perspective with black bars of height
 * 60 at the top and the bottom being inserted to maintain 16:9 aspect ratio.
 * Need to report raw grid as 1920x1080 and compensate y coordinate by 60 to
 * reflect the coordinates on a logical 1080p surface as reported to the apps
 */
#define FHD_PANEL_MAX_Y      1079
#define FHD_PANEL_OFFSET_Y   60
/*
 * APTIV panel true touch grid is 4096x4096 for a 1280x800 panel. To maintain
 * 16:9 aspect ratio, black bars of height 40 are being inserted at the top
 * and bottom. So, usable touch grid size is only 4096x3685 (4096-4096/800*80).
 * Need to report raw grid as 4096x3685 and compensate y coordinate by 205 to
 * reflect the coordinates on a logical 1080p surface as reported to the apps
 */
#define APTIV_PANEL_MAX_Y    3685
#define APTIV_PANEL_OFFSET_Y 205
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if (status & MXT_T100_DETECT) {
		...
		if (display_panel_type == FHD)
			y -= FHD_PANEL_OFFSET_Y;

		if (id >= MXT_MIN_RPTID_SEC) {
			input_mt_report_slot_state(input_dev_sec,
							tool,
							1);
			input_report_abs(input_dev_sec,
							ABS_MT_POSITION_X,
							x);
			input_report_abs(input_dev_sec,
							ABS_MT_POSITION_Y,
			...
			if (id == MXT_MIN_RPTID_SEC) {
				...
				input_report_abs(input_dev_sec,
								ABS_Y,
								y);
				input_report_key(input_dev_sec,
								BTN_TOUCH,
								1);
			}
...
static int mxt_update_cfg(struct mxt_data *data, const struct firmware *fw)
{
	...
	//if the panel is Aptiv, no need to update config
	if (display_panel_type == APTIV)
		return 0;
...
static int mxt_read_info_block(struct mxt_data *data)
{
	...
	for (count = 0; count < 20 ; count++) {
		...
		if (error) {
			...
			printk(KERN_ERR "retry success count =%d\n", count);
			msleep(100);
			break;
		}
	}
...
static int mxt_read_t100_config(struct mxt_data *data, u8 instance)
{
	...
	if (instance == 2) {
		T100_enable = object->start_address +
					mxt_obj_size(object) +
					MXT_T100_CTRL;
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	irq_gpio = of_get_named_gpio_flags(np,
		"irq-gpios", 0, &irq_flags);
#if 0
	irq_gpio = devm_gpiod_get_optional(&client->dev,
						   "irq", GPIOD_IN);
	if (IS_ERR(irq_gpio)) {
	...
	data->irq = client->irq;
	dev_err(&client->dev, "valid interrupt GPIO:%d irq_flags %x\n",
		client->irq,
		irq_flags);
	...
	/*read type */
	type = irq_get_trigger_type(client->irq);
	dev_err(&client->dev, "irq type = %08x\n", type);
