...
static ssize_t fwu_sysfs_utility_config_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(configid, 0444,
			fwu_sysfs_utility_config_id_show,
			synaptics_rmi4_store_error),
};
...
static int fwu_get_device_config_id(void)
{
	...
	dev_info(rmi4_data->pdev->dev.parent,
			"SYNAPTICS_FW_UPDATE fwu->config_id is %x%x\n",
			fwu->config_id[8],
			fwu->config_id[9]);
	if (retval < 0)
		return retval;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!mutex_trylock(&fwu_sysfs_mutex))
		return -EBUSY;
	retval = snprintf(buf, PAGE_SIZE, "%x%x\n",
					  fwu->config_id[8],
					  fwu->config_id[9]);
