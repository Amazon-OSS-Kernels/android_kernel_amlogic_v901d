		...
		bool binv)
{
	...
	/* This is just a copy from previous setting. */
	...
	unsigned int playback_active)
{
	...
	case SND_SOC_DAIFMT_CBS_CFS:
		if ((g_hwinfo_gpio_val == 0) && (index == TDM_A) &&
		(g_audio_clk_mode == 0)) {
			/* lagunaf cpu1:master cpu2:slave */
			valb = SLAVE_A + id;
			valf = SLAVE_A + id;
			master_mode = 0;
			pr_info("tdma is slave mode!\n");
		} else {
	...
	pr_info("master_mode(%d), binv_c(%d):_p(%d), finv(%d) out_skew(%d), in_skew(%d)\n",
			master_mode, binv_cap, binv_play,
			finv, bclkout_skew, bclkin_skew);
