...
static int get_reboot_reason(void)
{
	...
	reboot_reason = ((aml_read_aobus(0x08f << 2)>>12)&0xf);
	pr_info("%s, reboot mode = 0x%x!\n", __func__, reboot_reason);
...
static void switch_work_func(struct work_struct *work)
{
	...
	p_tdm = container_of(work, struct aml_tdm, work);
	if (p_tdm->last_audio_state != g_audio_clk_mode) {
		if (g_audio_clk_mode == 1 && !IS_ERR_OR_NULL(p_tdm->pin_ctl)) {
		//g_hwinfo_gpio_val = 1;
			if (!IS_ERR(p_tdm->mclk2pad)) {
				ret = clk_set_parent(p_tdm->mclk2pad,
					p_tdm->mclk);
				if (ret) {
					pr_info("Can't set tdm mclk_pad parent\n");
				return;
				}
			...
			#if 1
			p_tdm->pin_ctl =
			devm_pinctrl_get_select(p_tdm->dev, "tdm_pins");
			if (IS_ERR(p_tdm->pin_ctl))
				dev_warn_once(p_tdm->dev,
				"aml_tdm_get_master_pins error!\n");
			#endif

			aml_set_default_tdm_clk(p_tdm);

		} else if (g_audio_clk_mode == 0 &&
			!IS_ERR_OR_NULL(p_tdm->pin_ctl)) {
			//g_hwinfo_gpio_val = 0;

			#if 1
			pr_info("pincrl slave mode\n");
			p_tdm->pin_ctl = devm_pinctrl_get_select(p_tdm->dev,
				"tdm_slv_pins");
			if (IS_ERR(p_tdm->pin_ctl))
				dev_warn_once(p_tdm->dev,
					"aml_tdm_get_slave_pins error!\n");
			#endif

			ps = pinctrl_lookup_state(p_tdm->pin_ctl,
				"tdmout_a_gpio");
		...
		aml_tdm_set_format(p_tdm->actrl,
		&(p_tdm->setting), p_tdm->clk_sel, p_tdm->id, 0x4011,
		1, 1);
		p_tdm->last_audio_state = g_audio_clk_mode;
	}
}

static void switch_audio_start_timer(struct aml_tdm *p_tdm,
				  unsigned long delay)
{
	p_tdm->timer.expires = jiffies + delay;
	add_timer(&p_tdm->timer);
}

static void switch_audio_stop_timer(struct aml_tdm *p_tdm)
{
	del_timer_sync(&p_tdm->timer);
	cancel_work_sync(&p_tdm->work);
}


static void switch_timer_func(unsigned long data)
{
	struct aml_tdm *p_tdm = (struct aml_tdm *)data;
	unsigned long delay = msecs_to_jiffies(150);

	schedule_work(&p_tdm->work);
	mod_timer(&p_tdm->timer, jiffies + delay);
}

static void audio_switch_detect(struct aml_tdm *p_tdm)
{
	init_timer(&p_tdm->timer);
	p_tdm->timer.function = switch_timer_func;
	p_tdm->timer.data = (unsigned long)p_tdm;

	INIT_WORK(&p_tdm->work, switch_work_func);

	switch_audio_start_timer(p_tdm,
...
static int aml_tdm_platform_probe(struct platform_device *pdev)
{
	...
	if (p_tdm->id == TDM_A) {
		if (g_hwinfo_gpio_val == 1 && get_reboot_reason() != 0x2) {
			/* master mode */
			p_tdm->pin_ctl =
				devm_pinctrl_get_select(dev, "tdm_pins");
			if (IS_ERR(p_tdm->pin_ctl))
				dev_warn_once(dev, "aml_tdm_get_master_pins error!\n");
			else
				dev_info(dev, "tdma is as master mode!\n");
		} else if (g_hwinfo_gpio_val == 0) {
			/* slave mode */
			p_tdm->pin_ctl =
				devm_pinctrl_get_select(dev, "tdm_slv_pins");
			if (IS_ERR(p_tdm->pin_ctl))
				dev_warn_once(dev, "aml_tdm_get_slv_pins error!\n");
			else
				dev_info(dev, "tdma is as slave mode!\n");
		} else
			dev_err_once(dev, "error: aml tdm can't get hwinfo gpio!\n");

		if (g_hwinfo_gpio_val == 0) {
			/* disable mclk for slave mode */
			if (!IS_ERR(p_tdm->mclk2pad))
				clk_disable_unprepare(p_tdm->mclk2pad);
		}
	} else {
	...
	if (ret < 0)
		...
		if ((p_tdm->id == TDM_A) && (g_hwinfo_gpio_val == 0))
			p_tdm->start_clk_enable = 0;
